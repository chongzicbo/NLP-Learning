...
if __name__ == "__main__":
    ...
    runner_config = {"ckpt_period": 1, "model_dir": "/tmp/dssm"}
    ...
    dataset = XYZSameFileDataset(
        x_tokenizer=tokenizer, y_tokenizer=tokenizer, config=None
    )

    runner = KerasModelDatasetRunner(
        model=model, model_name="dssm", model_dir=config["model_dir"], configs=config
    )
    ...
    if "train" == args.action:
        ...
        eval_dataset = (
            dataset.build_eval_dataset(eval_files=config["eval_files"])
