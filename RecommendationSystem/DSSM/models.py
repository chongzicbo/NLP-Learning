...
def build_mlp_model(config):
    query = keras.layers.Input(
        shape=(config["query_max_len"],), dtype=tf.int64, name="query_input"
    )
    doc = keras.layers.Input(
        shape=(config["doc_max_len"],), dtype=tf.int64, name="doc_input"
    )

    embedding = keras.layers.Embedding(
        config["vocab_size"], config["embedding_size"], name="embedding"
    )
    # average embedding
