            ...
            if i != len(pos_list) - 1:
                train_set.append(
                    (
                        ...
                        pos_list[i],
                        1,
                        hist[::-1][:seq_short_len][::-1],
                        hist[::-1][seq_short_len : seq_short_len + seq_prefer_len],
                        seq_short_len,
                        seq_prefer_len,
                        genres_hist[::-1][:seq_short_len][::-1],
                        genres_hist[::-1][
                            seq_short_len : seq_short_len + seq_prefer_len
                        ],
                        rating_list[i],
                    )
                )
...
def gen_model_input(train_set, user_profile, seq_max_len):
    ...
    train_seq_genres_pad = pad_sequences(
        train_seq_genres, maxlen=seq_max_len, padding="post", truncating="post", value=0
    )
    train_model_input = {
        "user_id": train_uid,
        "movie_id": train_iid,
    ...
    for key in ["gender", "age", "occupation", "zip"]:
        train_model_input[key] = user_profile.loc[train_model_input["user_id"]][
            key
        ].values
...
def gen_model_input_sdm(train_set, user_profile, seq_short_max_len, seq_prefer_max_len):
    ...
    train_short_item_pad = pad_sequences(
        short_train_seq,
        maxlen=seq_short_max_len,
        padding="post",
        truncating="post",
        value=0,
    )
    train_prefer_item_pad = pad_sequences(
        prefer_train_seq,
        maxlen=seq_prefer_max_len,
        padding="post",
        truncating="post",
        value=0,
    )
    train_short_genres_pad = pad_sequences(
    ...
    train_prefer_genres_pad = pad_sequences(
        prefer_train_seq_genres,
        maxlen=seq_prefer_max_len,
        padding="post",
        truncating="post",
        value=0,
