...
simbert = build_transformer_model(
    sim_config_path, sim_checkpoint_path, with_pool="linear", application="unilm"
).to(device)
...
def collate_fn(batch):
    ...
    # ==== 蒸馏用：开始 ====
    batch_sim_token_ids = torch.tensor(
        sequence_padding(batch_sim_token_ids), dtype=torch.long, device=device
    )
    batch_sim_segment_ids = torch.tensor(
        sequence_padding(batch_sim_segment_ids), dtype=torch.long, device=device
    )
...
train_dataloader = DataLoader(
    MyDataset("../datasets/data_similarity.json"),
...
class TotalLoss(nn.Module):
    ...
    def forward(self, outputs, target):
        ...
        return {
            "loss": seq2seq_loss + similarity_loss,
...
def just_show(some_samples):
    ...
    for s in S:
        try:
            print("原句子：%s" % s)
            print("同义句子：", gen_synonyms(s, 10, 10))
            print()
        except:
            pass
