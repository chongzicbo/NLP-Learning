...
@author: chengbo
@software: PyCharm
@file: task_relation_extraction_tplinker.py
@time: 2022/8/10 21:50
"""
...
def collate_fn(batch):
    ...
    batch_entity_labels = torch.zeros(
        (len(batch), pair_len), dtype=torch.long, device=device
    )
    batch_head_labels = torch.zeros(
        (len(batch), len(predicate2id), pair_len), dtype=torch.long, device=device
    )
...
class Model(BaseModel):
    def __init__(self):
        ...
        self.bert = build_transformer_model(
            config_path, checkpoint_path, segment_vocab_size=0
        )
    ...
    def forward(self, inputs):
        ...
        shaking_hiddens = self.handshaking_kernel(
            last_hidden_state
        )  # [btz, pair_len, hdsz]
        ent_shaking_outputs = self.ent_fc(shaking_hiddens)  # [btz, pair_len, 2]

        btz, pair_len = shaking_hiddens.shape[:2]
        head_rel_shaking_outputs = self.head_rel_fc(shaking_hiddens).reshape(
            btz, -1, pair_len, 3
        )  # [btz, predicate_num, pair_len, 3]
        tail_rel_shaking_outputs = self.tail_rel_fc(shaking_hiddens).reshape(
            btz, -1, pair_len, 3
        )
...
class MyLoss(nn.CrossEntropyLoss):
    ...
    def forward(self, y_preds, y_trues):
        ...
        w_rel = min(
            (len(predicate2id) / z) * model.global_step / total_steps,
            (len(predicate2id) / z),
        )
...
def extract_spoes(text):
    ...
    token_ids_ts = torch.tensor(
        sequence_padding([token_ids], length=maxlen), dtype=torch.long, device=device
    )
...
def evaluate(data):
    ...
    for d in data:
        ...
        # 计算实体的指标
        ent_text_truth = set(
            [spo[0] for spo in spo_list] + [spo[-1] for spo in spo_list]
        )
