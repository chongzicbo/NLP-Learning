...
@author: chengbo
@software: PyCharm
@file: task_question_answer_generation_by_seq2seq.py
@time: 2022/8/10 23:17
"""
# 用Seq2Seq做阅读理解构建
...
def process_data():
    ...
    json.dump(
        train_data,
        open("F:/Projects/data/corpus/qa/CIPS-SOGOU/train_data_list_format.json", "w"),
        indent=4,
    )
    json.dump(
        valid_data,
        open("F:/Projects/data/corpus/qa/CIPS-SOGOU/valid_data_list_format.json", "w"),
        indent=4,
    )
...
def collate_fn(batch):
    ...
    for p, q, a in batch:
        ...
        token_ids = (
            p_token_ids + a_token_ids[1:] + q_token_ids[1:]
...
train_dataloader = DataLoader(
    MyDataset("F:/Projects/data/corpus/qa/CIPS-SOGOU/train_data_list_format.json"),
...
class QuestionAnswerGeneration(AutoRegressiveDecoder):
    ...
    def generate(self, passage, topk=1, topp=0.95):
        ...
        return (
            tokenizer.decode(q_ids.cpu().numpy()),
            tokenizer.decode(a_ids.cpu().numpy()),
        )


qag = QuestionAnswerGeneration(
    start_id=None, end_id=tokenizer._token_end_id, maxlen=max_q_len, device=device
)
