...
@author: chengbo
@software: PyCharm
@file: task_reading_comprehension_by_seq2seq.py
@time: 2022/8/10 23:13
"""
# 用seq2seq的方式做阅读理解任务
...
class ReadingComprehension(AutoRegressiveDecoder):
    ...
    def predict(self, inputs, output_ids, states):
        ...
        index_ = (
            torch.tensor([[len(i) - 1] for i in all_token_ids], device=probas.device)
            .view(-1, 1, 1)
            .expand(-1, 1, probas.shape[-1])
        )
        probas = torch.gather(probas, dim=1, index=index_).reshape(
            (len(inputs), topk, -1)
        )
        ...
        if self.mode == "extractive":
            ...
            for token_ids in inputs:
                ...
                for k, v in self.get_ngram_set(
                    p_token_ids.cpu().numpy(), states + 1
                ).items():  # 这里要放到.cpu().numpy()，否则会出现nrams.get不到
