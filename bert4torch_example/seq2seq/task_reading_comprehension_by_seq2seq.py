...
class ReadingComprehension(AutoRegressiveDecoder):
    ...
    def predict(self, inputs, output_ids, states):
        ...
        index_ = torch.tensor([[len(i) - 1] for i in all_token_ids], device=probas.device).view(-1, 1, 1).expand(-1, 1,
                                                                                                                 probas.shape[
                                                                                                                     -1])
        ...
        if self.mode == 'extractive':
            ...
            for token_ids in inputs:
                ...
                for k, v in self.get_ngram_set(p_token_ids.cpu().numpy(),
                                               states + 1).items():  # 这里要放到.cpu().numpy()，否则会出现nrams.get不到
