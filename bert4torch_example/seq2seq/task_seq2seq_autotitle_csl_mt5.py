...
@author: chengbo
@software: PyCharm
@file: task_seq2seq_autotitle_csl_mt5.py
@time: 2022/8/10 23:39
"""
...
def collate_fn(batch):
    ...
    batch_content_ids = torch.tensor(
        sequence_padding(batch_content_ids, value=token_pad_ids),
        dtype=torch.long,
        device=device,
    )
    batch_titile_ids = torch.tensor(
        sequence_padding(batch_titile_ids, value=token_pad_ids),
        dtype=torch.long,
        device=device,
    )
...
model.compile(
    loss=CrossEntropyLoss(ignore_index=token_pad_ids),
