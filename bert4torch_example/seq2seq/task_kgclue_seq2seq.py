...
@author: chengbo
@software: PyCharm
@file: task_kgclue_seq2seq.py
@time: 2022/8/10 23:19
"""
...
train_dataloader = DataLoader(
    ListDataset(train_data), shuffle=True, collate_fn=collate_fn
)
...
model = build_transformer_model(
    config_path, checkpoint_path, model="roformer", application="unilm"
).to(device)
...
class AutoQA(AutoRegressiveDecoder):
            ...
            if len(next_ids) > 1 and self.end_id in ids:  # 容许集大于1且已解码出S
                ...
                for c in candidates:
                    if len(c) > len(ids):
                        ...
                        weights[c[len(ids)]] = max(
                            w + 1, weights[c[len(ids)]].cpu().numpy()
                        )
                probas[i] = torch.pow(probas[i], 1.0 / weights)  # 按 p^(1/n) 来增大权重
    ...
    def generate(self, text, topk=1):
        ...
        output_ids = self.beam_search(
            [token_ids, segment_ids], topk=topk, min_ends=3
        )  # 基于beam search
        ...
        return (
            tokenizer.decode(subject_ids.cpu().numpy()),
            tokenizer.decode(predicate_ids.cpu().numpy()),
            tokenizer.decode(meaning_ids.cpu().numpy()),
            KG[output_ids[:-1].cpu().numpy()],
        )
...
class Evaluator(Callback):
    ...
    def f1sim(self, text_a, text_b):
        ...
        if not text_a and not text_b:
            ...
            return 2.0 * lcs_len / (len(text_a) + len(text_b))
...
if __name__ == "__main__":
    ...
    print("[TEST] topk=1, em: %.5f, f1: %.5f, score: %.5f" % (em, f1, score))
    em, f1, score = evaluator.evaluate(test_data, topk=3)
    print("[TEST] topk=3, em: %.5f, f1: %.5f, score: %.5f" % (em, f1, score))
    em, f1, score = evaluator.evaluate(test_data, topk=5)
    print("[TEST] topk=5, em: %.5f, f1: %.5f, score: %.5f" % (em, f1, score))
