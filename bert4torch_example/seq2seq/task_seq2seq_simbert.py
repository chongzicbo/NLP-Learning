...
@author: chengbo
@software: PyCharm
@file: task_seq2seq_simbert.py
@time: 2022/8/10 23:12
"""
...
class TotalLoss(nn.Module):
    ...
    def compute_loss_of_similarity(self, y_pred):
        ...
        similarities = (
            similarities - torch.eye(y_pred.shape[0], device=device) * 1e12
        )  # 排除对角线
...
if __name__ == "__main__":
    ...
    if choice == "train":
        evaluator = Evaluator()
        model.fit(
            train_dataloader, epochs=50, steps_per_epoch=200, callbacks=[evaluator]
        )
        ...
        for i, line in enumerate(text_list):
            print(
                f'cos_sim: {similarity[i].item():.4f}, tgt_text: "{target_text}", cal_text: "{line}"'
            )
