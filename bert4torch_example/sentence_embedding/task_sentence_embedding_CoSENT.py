...
@author: chengbo
@software: PyCharm
@file: task_sentence_embedding_CoSENT.py
@time: 2022/8/10 22:34
"""
...
class MyLoss(nn.Module):
    def forward(self, y_pred, y_true):
        ...
        y_pred = torch.cat(
            (torch.tensor([0.0], device=device), y_pred), dim=0
...
class Evaluator(Callback):
    ...
    def on_epoch_end(self, global_step, epoch, logs=None):
        ...
        print(
            f"test_consine: {val_consine:.5f}, best_test_consine: {self.best_val_consine:.5f}\n"
        )
