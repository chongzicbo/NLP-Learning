...
@author: chengbo
@software: PyCharm
@file: task_sentence_embedding_unsup_PromptBert.py
@time: 2022/8/12 9:45
"""
...
mask_token = "[MASK]"
prompt_templates = [
    '"{}" 的意思为[MASK]'.format(replace_token),
    ...
    for filename in filenames:
        with open(filename, "r", encoding="utf-8") as f:
            for line in tqdm(f.readlines(), desc="Load data"):
                cache = line.split("\t")
                text1, text2, label = (
                    cache[0][: maxlen - template_len],
                    cache[1][: maxlen - template_len],
                    cache[-1],
                )
                for text in [text1, text2]:
                    ...
                    for template in prompt_templates:
                        ...
                        template_sent = template.replace(
                            replace_token, replace_token * sent_num
                        )
...
def collate_fn(batch):
    ...
    for i, item in enumerate(batch_tensor):
        batch_tensor[i] = torch.tensor(
            sequence_padding(item, maxlen), dtype=torch.long, device=device
        )
...
def collate_fn_test(batch):
    ...
    text1_ids = torch.tensor(
        sequence_padding(text1_ids), dtype=torch.long, device=device
    )
    text2_ids = torch.tensor(
        sequence_padding(text2_ids), dtype=torch.long, device=device
    )
...
valid_dataloader = DataLoader(
    ListDataset(data=valid_texts), batch_size=batch_size, collate_fn=collate_fn_test
)
...
class PromptBert(BaseModel):
    ...
    def get_mask_embedding(self, input_ids):
        ...
        input_mask_expanded = (
            mask_index.unsqueeze(-1).expand(last_hidden_state.size()).float()
        )
