...
@author: chengbo
@software: PyCharm
@file: task_sentence_embedding_unsup_ESimCSE.py
@time: 2022/8/12 9:45
"""
# ESimCSE 中文测试
...
class CollateFunc(object):
    ...
    def __call__(self, batch_text):
        ...
        for text, text_pos in zip(batch_text, batch_pos_text):
            ...
            batch_pos_tokens_list.append(
                self.tokenizer.encode(text_pos, maxlen=maxlen)[0]
            )

        batch_neg_tokens_list = []
        if batch_neg_text:
            for text in batch_neg_text:
                batch_neg_tokens_list.append(
                    self.tokenizer.encode(text, maxlen=maxlen)[0]
                )

        batch_tokens_list = torch.tensor(
            sequence_padding(batch_tokens_list), dtype=torch.long, device=device
        )
        batch_pos_tokens_list = torch.tensor(
            sequence_padding(batch_pos_tokens_list), dtype=torch.long, device=device
        )

        labels = torch.arange(
            batch_tokens_list.size(0), device=batch_tokens_list.device
        )
        if batch_neg_tokens_list:
            batch_neg_tokens_list = torch.tensor(
                sequence_padding(batch_neg_tokens_list), dtype=torch.long, device=device
            )
            return [
                batch_tokens_list,
...
class Momentum(object):
    ...
    def step(self):
        for encoder_param, moco_encoder_param in zip(
            model.encoder.parameters(), model.momentum_encoder.parameters()
        ):
            moco_encoder_param.data = (
                self.gamma * moco_encoder_param.data
                + (1.0 - self.gamma) * encoder_param.data
            )
