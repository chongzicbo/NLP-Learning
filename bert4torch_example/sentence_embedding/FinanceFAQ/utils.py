...
def cos_sim4matrix(arr, brr):
    return 0.5 + 0.5 * (
        arr.dot(brr.T)
        / (np.sqrt(np.sum(arr * arr)) * np.sqrt(np.sum(brr * brr, axis=1)))
    )


def cos_sim4matrix_2(arr, brr):
    return arr.dot(brr.T) / (
        np.sqrt(np.sum(arr * arr)) * np.sqrt(np.sum(brr * brr, axis=1))
    )


def read_q_std_q_corpus(
    q_std_file, q_std_vectors_file, q_corpus_file, q_corpus_vectors_file
):
    ...
    q_all_sentence_embeddings_dict = {
        q_all[i]: q_all_sentence_embeddings[i] for i in range(0, len(q_all))
    }
...
def cal_performance(
    ...
    texts_embeddings_dict_1 = {
        texts_in[i]: q_all_sentence_embeddings_dict[texts_in[i]]
        for i in range(0, len(texts_in))
    }
    texts_embeddings_dict_2 = {
        texts_out[i]: texts_out_embeddings[i] for i in range(0, len(texts_out))
    }
    ...
    print(
        "shape: ",
        x_texts_embeddings.shape,
        q_std_sentence_embeddings.shape,
        cos_scores.shape,
    )

    print(f"----为每条相似问找到相似度最大的{K}条标问".center(60, "-"))
    cos_scores_top_k_values, cos_scores_top_k_idx = torch.topk(
        cos_scores, K, dim=1, largest=True, sorted=False
    )
    cos_scores_top_k_values = cos_scores_top_k_values.tolist()
    cos_scores_top_k_idx = cos_scores_top_k_idx.tolist()
    cos_q_corpus_sort = [
        [q_std_list[v] for v in vlist] for vlist in cos_scores_top_k_idx
    ]  # 最相似的TopK个标问
    result = [
        list(zip(cos_q_corpus_sort[i], cos_scores_top_k_values[i]))
    ...
    # 计算q_sim和q_std之间的相似度
    df_eval["prob_with_std"] = df_eval.apply(
        lambda row: cos_sim_1(
            texts_embeddings_dict[row["q_sim"]],
            q_std_sentence_embeddings[q_std_list.index(row["q_std"])],
        ),
        axis=1,
    )
    ...
    df_eval["q_std_pred_list_v1"] = df_eval.q_std_pred_list.apply(
        lambda v: [k[0] for k in v]
    )  # 只保留预测的标准问句
    df_eval["q_std_pred_list_v2"] = df_eval.q_std_pred_list.apply(
        lambda v: [k[1] for k in v]
    )  # 只保留预测的概率
    df_eval["t1"] = df_eval.apply(
        lambda row: 1 if row["q_std"] in row["q_std_pred_list_v1"][0:1] else 0, axis=1
    )
    df_eval["t3"] = df_eval.apply(
        lambda row: 1 if row["q_std"] in row["q_std_pred_list_v1"][0:3] else 0, axis=1
    )
    df_eval["t5"] = df_eval.apply(
        lambda row: 1 if row["q_std"] in row["q_std_pred_list_v1"][0:5] else 0, axis=1
    )
    df_eval["t10"] = df_eval.apply(
        lambda row: 1 if row["q_std"] in row["q_std_pred_list_v1"][0:10] else 0, axis=1
    )

    print(
        "----模型准确率: ",
        df_eval.t1.sum() / df_eval.shape[0],
        df_eval.t3.sum() / df_eval.shape[0],
        df_eval.t5.sum() / df_eval.shape[0],
        df_eval.t10.sum() / df_eval.shape[0],
    )
    df_eval_need = df_eval[df_eval.ifin == 1]
    print(
        "----模型准确率:[有效标问]：",
        df_eval_need.t1.sum() / df_eval_need.shape[0],
        df_eval_need.t3.sum() / df_eval_need.shape[0],
        df_eval_need.t5.sum() / df_eval_need.shape[0],
        df_eval_need.t10.sum() / df_eval_need.shape[0],
    )
    return df_eval
