...
@author: chengbo
@software: PyCharm
@file: task_sentence_embedding_FinanceFAQ_step2_1.py
...
def collate_fn(batch):
    ...
    for i, token_ids in enumerate(tokens_ids_list):
        tokens_ids_list[i] = torch.tensor(
            sequence_padding(token_ids), dtype=torch.long, device=device
        )
...
class Model(BaseModel):
    ...
    def forward(self, token_ids_list):
        ...
        for token_ids in token_ids_list:
            ...
            rep = get_pool_emb(
                hidden_state1, pool_cls1, token_ids.gt(0).long(), self.pool_method
            )
            reps.append(rep)
        embeddings_a = reps[0]
    ...
    def encode(self, texts):
        ...
        token_ids_tensor = torch.tensor(
            sequence_padding(token_ids_list), dtype=torch.long
        )
        valid_dataloader = DataLoader(
            TensorDataset(token_ids_tensor), batch_size=batch_size
        )
        valid_sen_emb = []
...
class Evaluator(Callback):
    ...
    def on_dataloader_end(self, logs=None):
        model.train_dataloader = DataLoader(
            MyDataset(sec_train_file),
...
if __name__ == "__main__":
    ...
    evaluator = Evaluator()
    model.fit(train_dataloader, epochs=10, steps_per_epoch=1000, callbacks=[evaluator])
else:
