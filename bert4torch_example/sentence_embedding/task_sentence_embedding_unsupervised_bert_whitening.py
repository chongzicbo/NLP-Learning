...
@author: chengbo
@software: PyCharm
@file: task_sentence_embedding_unsupervised_bert_whitening.py
@time: 2022/8/10 22:43
"""
...
train_dataloader = DataLoader(
    MyDataset(all_names), batch_size=batch_size, shuffle=True, collate_fn=collate_fn
)
...
class Model(BaseModel):
    ...
    def encode(self, token_ids):
        ...
        with torch.no_grad():
            ...
            output = get_pool_emb(
                hidden_state, pool_cls, attention_mask, self.pool_method
            )
        return output
...
corrcoef = scipy.stats.spearmanr(
    sen_labels.cpu().numpy(), all_sims.cpu().numpy()
).correlation
