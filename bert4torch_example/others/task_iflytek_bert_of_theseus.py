...
@author: chengbo
@software: PyCharm
@file: task_iflytek_bert_of_theseus.py
@time: 2022/8/11 18:43
"""
# 文本分类例子下的模型压缩
# 方法为BERT-of-Theseus
...
class BERT_THESEUS(BERT):
    def __init__(self, **kwargs):
        ...
        self.encoderLayer = nn.ModuleList(
            nn.ModuleList([copy.deepcopy(layer) for _ in range(self.num_hidden_layers)])
        )
        self.scc_n_layer = 6  # 蒸馏到6层
        self.scc_layer = nn.ModuleList(
            [copy.deepcopy(layer) for _ in range(self.scc_n_layer)]
        )
                ...
                if self.bernoulli.sample() == 1:  # REPLACE
                    ...
                    for offset in range(self.compress_ratio):
                        inference_layers.append(
                            self.encoderLayer[i * self.compress_ratio + offset]
                        )
...
class Model(BaseModel):
    def __init__(self) -> None:
        ...
        self.bert = build_transformer_model(
            config_path=config_path, checkpoint_path=checkpoint_path, model=BERT_THESEUS
        )
...
class LinearReplacementScheduler:
    ...
    def step(self):
        ...
        current_replacing_rate = min(
            self.k * self.step_counter + self.base_replacing_rate, 1.0
        )
...
replacing_rate_scheduler = ConstantReplacementScheduler(
    ...
    replacing_steps=steps_for_replacing,
)
model.compile(
    loss=nn.CrossEntropyLoss(),
    optimizer=optim.Adam(model.parameters(), lr=2e-5),
    scheduler=replacing_rate_scheduler,
    metrics=["accuracy"],
)
