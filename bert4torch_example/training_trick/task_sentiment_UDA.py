...
unsup_dataset = [
    sen for sen, _ in (train_dataset.data + valid_dataset.data + test_dataset.data)
]
...
def collate_fn(batch):
    ...
    for text, label in batch:
        ...
        batch_token_ids[2].append(
            token_ids[:1] + add_noise(token_ids[1:-1]) + token_ids[-1:]
        )  # 无监督数据增强
...
class Loss(UDALoss):
    def forward(self, y_pred, y_true_sup):
        loss, loss_sup, loss_unsup = super().forward(
            y_pred, y_true_sup, model.global_step, model.total_steps
        )
...
model.compile(
    loss=Loss(
        tsa_schedule="linear_schedule", start_p=0.8
