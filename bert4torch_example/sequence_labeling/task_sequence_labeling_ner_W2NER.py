...
@author: chengbo
@software: PyCharm
@file: task_sequence_labeling_ner_W2NER.py
@time: 2022/9/17 17:37
"""
        ...
        with open(filename, encoding="utf-8") as f:
            ...
            for l in tqdm(f.split("\n\n"), desc="Load data"):
                ...
                if len(sentence) > maxlen - 2:
                    continue
                tokens = [
                    tokenizer.tokenize(word)[1:-1] for word in sentence[: maxlen - 2]
                ]
                ...
                _entity_text = set(
                    [
                        convert_index_to_text(
                            list(range(e[0], e[1] + 1)), categories[e[-1]]
                        )
                        for e in d
                    ]
                )
...
def collate_fn(data):
    tokens_ids, pieces2word, dist_inputs, grid_labels, grid_mask2d, _entity_text = map(
        list, zip(*data)
    )

    sent_length = torch.tensor(
        [i.shape[0] for i in pieces2word], dtype=torch.long, device=device
    )
    ...
    tokens_ids = torch.tensor(
        sequence_padding(tokens_ids), dtype=torch.long, device=device
    )
    batch_size = tokens_ids.size(0)

    def fill(data, new_data):
        for j, x in enumerate(data):
            new_data[j, : x.shape[0], : x.shape[1]] = torch.tensor(
                x, dtype=torch.long, device=device
            )
        return new_data

    dis_mat = torch.zeros(
        (batch_size, max_wordlen, max_wordlen), dtype=torch.long, device=device
    )
    ...
    mask2d_mat = torch.zeros(
        (batch_size, max_wordlen, max_wordlen), dtype=torch.bool, device=device
    )
    grid_mask2d = fill(grid_mask2d, mask2d_mat)
    sub_mat = torch.zeros(
        (batch_size, max_wordlen, max_tokenlen), dtype=torch.bool, device=device
    )
...
class CoPredictor(nn.Module):
    def __init__(
        self, cls_num, hid_size, biaffine_size, channels, ffnn_hid_size, dropout=0
    ):
        super().__init__()
...
class Model(BaseModel):
    def __init__(self, use_bert_last_4_layers=False):
        ...
        self.convLayer = ConvolutionLayer(
            conv_input_size, conv_hid_size, dilation, conv_dropout
        )
    ...
    def forward(self, token_ids, pieces2word, dist_inputs, sent_length, grid_mask2d):
        ...
        packed_embs = pack_padded_sequence(
            word_reps, sent_length.cpu(), batch_first=True, enforce_sorted=False
        )
...
scheduler = get_linear_schedule_with_warmup(
    optimizer,
    num_warmup_steps=warm_factor * updates_total,
...
class Evaluator(Callback):
    ...
    def evaluate(self, data_loader):
        ...
        for data_batch in tqdm(data_loader, desc="Evaluate"):
            ...
            ent_c, ent_p, ent_r, _ = self.decode(
