...
class UIEPredictor(object):
            ...
            if not node.prefix:
                ...
                for pre, data in zip(node.prefix, datas):
                    if len(pre) == 0:
                        ...
                        for p in pre:
                            examples.append(
                                {"text": data, "prompt": dbc2sbc(p + node.name)}
                            )
    ...
    def _single_stage_predict(self, inputs):
        ...
        token_ids, segment_ids, offset_maps = self._tokenizer.encode(
            short_texts_prompts,
            short_input_texts,
        ...
        for batch_start in range(0, len(short_input_texts), self._batch_size):
            ...
            batch_token_ids = torch.tensor(
                sequence_padding(batch_token_ids), dtype=torch.long, device=self._device
            )
            batch_segment_ids = torch.tensor(
                sequence_padding(batch_segment_ids),
                dtype=torch.long,
                device=self._device,
            )

            start_prob, end_prob = self.model.predict(
        ...
        for start_ids, end_ids, ids, offset_map in zip(
            start_ids_list, end_ids_list, token_ids, offset_maps
        ):
    ...
    def _build_tree(cls, schema, name="root"):
        ...
        for s in schema:
            if isinstance(s, str):
                ...
                raise TypeError(
                    "Invalid schema, element should be string or dict, but {} received".format(
                        type(s)
                    )
                )
        return schema_tree
