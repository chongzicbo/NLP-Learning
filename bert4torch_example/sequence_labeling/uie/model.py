    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get("use_task_id") and kwargs.get("use_task_id"):
            ...
            task_type_embeddings = nn.Embedding(
                kwargs.get("task_type_vocab_size"), self.hidden_size
            )
            ...
            def hook(module, input, output):
                return output + task_type_embeddings(
                    torch.zeros(
                        input[0].size(), dtype=torch.int64, device=input[0].device
                    )
                )
...
uie_model = build_transformer_model(
    config_path=config_path, checkpoint_path=checkpoint_path, model=UIE, with_pool=True
)
