...
def collate_fn(batch):
    ...
    batch_token_ids = torch.tensor(sequence_padding(batch_token_ids, length=max_segment), dtype=torch.long,
                                   device=device)
...
def do_train(df_train):
    ...
    for fold, (trn_idx, val_idx) in enumerate(skf.split(df_train['text'], df_train['label']), 1):
        ...
        train_dataloader = DataLoader(ListDataset(data=train_data), batch_size=batch_size, shuffle=True,
