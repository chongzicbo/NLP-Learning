...
def collate_fn(batch):
    ...
    batch_token_ids = torch.tensor(
        sequence_padding(batch_token_ids, length=max_segment),
        dtype=torch.long,
        device=device,
    )
...
class Model(BaseModel):
    ...
    def forward(self, token_ids):
        ...
        input_mask = torch.any(
            token_ids, dim=-1, keepdim=True
        ).long()  # [btz, max_segment, 1]
...
def do_train(df_train):
    ...
    for fold, (trn_idx, val_idx) in enumerate(
        skf.split(df_train["text"], df_train["label"]), 1
    ):
        print(f"[Fold {fold}]")
        ...
        train_dataloader = DataLoader(
            ListDataset(data=train_data),
            batch_size=batch_size,
            shuffle=True,
            collate_fn=collate_fn,
        )
        valid_dataloader = DataLoader(
            ListDataset(data=valid_data), batch_size=batch_size, collate_fn=collate_fn
        )
        ...
        callbacks = [
            ...
            EarlyStopping(
                monitor="val_f1", patience=5, verbose=1, mode="max"
            ),  # 需要在Evaluator后面
        ]
        model.fit(
            train_dataloader,
