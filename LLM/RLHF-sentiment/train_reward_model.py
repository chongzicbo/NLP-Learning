...
parser.add_argument(
    "--model", default="bert-base-chinese", type=str, help="backbone of encoder."
)
parser.add_argument(
    "--train_path", default="data/train.tsv", type=str, help="The path of train set."
)
parser.add_argument(
    "--dev_path", default="data/dev.tsv", type=str, help="The path of dev set."
)
parser.add_argument(
    "--save_dir",
    default="./checkpoints",
...
parser.add_argument(
    "--valid_steps", default=200, type=int, required=False, help="evaluate frequecny."
)
parser.add_argument("--logging_steps", default=10, type=int, help="log interval.")
parser.add_argument(
    "--img_log_dir", default="logs", type=str, help="Logging image path."
)
parser.add_argument(
    "--img_log_name",
    default="Model Performance",
    type=str,
    help="Logging image file name.",
)
parser.add_argument(
    "--device",
    default="cuda:0",
        ...
        for batch in data_loader:
            for batch_idx in range(len(batch["input_ids"])):
                ...
                for text_idx in range(rank_texts_count):
                    reward = model(
                        batch["input_ids"][batch_idx][text_idx]
                        .unsqueeze(dim=0)
                        .to(args.device),
                        batch["token_type_ids"][batch_idx][text_idx]
                        .unsqueeze(dim=0)
                        .to(args.device),
                        batch["attention_mask"][batch_idx][text_idx]
                        .unsqueeze(dim=0)
                        .to(args.device),
                        batch["position_ids"][batch_idx][text_idx]
                        .unsqueeze(dim=0)
                        .to(args.device),
                    )
                    rank_rewards.append(
                        reward[0]
                    )  # (rank_text_num) -> [tensor([0.1696]), tensor([0.3466])]
...
def train():
    ...
    dataset = load_dataset(
        "text", data_files={"train": args.train_path, "dev": args.dev_path}
    )
    print(dataset)
