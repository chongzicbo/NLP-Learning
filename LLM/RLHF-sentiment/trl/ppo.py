...
class PPOTrainer:
    ...
    def step(self, queries, responses, scores):
        ...
        logprobs, ref_logprobs, values = self.batched_forward_pass(
            queries, responses
        )  # 拿到模型生成的tokens的log_prob、token_value
        ...
        for i in range(int(bs / fbs)):
            ...
            for j in range(fbs):
                ...
                all_ref_logprobs.append(
                    ref_logprobs[j, start:end]
                )  # ref model生成的tokens的概率
    ...
    def loss(self, old_logprobs, values, rewards, query, response, model_input):
        ...
        logits, _, vpred = self.model(
            model_input
        )  # logits -> (batch, all_seq_len, vocab_size); vpred -> (batch, all_seq_len)
        ...
        logprob, vpred = (
            logprob[:, -gen_len:],
            vpred[:, -gen_len - 1 : -1],
        )  # logprob -> (batch, generated_seq_len); vpred -> (batch, generated_seq_len)
