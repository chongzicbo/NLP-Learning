...
"""
# File       : ppo_sentiment_example.py
# Time       ：2023/4/21 11:30
# Author     ：chengbo
# version    ：python 3.8
# Description：
...
好评生成器训练：
1.使用GPT2模型采样训练样本
2.使用中文情感分析模型来给采样的样本打分并排序
3.使用强化学习ppo算法更新GPT2模型参数
"""
import os
import time
import random
...
# 情感分类模型
senti_tokenizer = AutoTokenizer.from_pretrained(
...
for epoch in tqdm(range(total_ppo_epochs)):
    ...
    for i in range(config["batch_size"]):
        gen_len = config["gen_len"]  # 生成的内容长度
        response = gpt2_model.generate(
            query_tensors[i].unsqueeze(dim=0),  # generate()用于直接生成token_id
    ...
    pipe_outputs = sentiment_pipe(texts)  # 情感模型计算得分
    rewards = []
