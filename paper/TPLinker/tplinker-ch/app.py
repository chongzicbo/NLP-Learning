...
device = (
    "cpu"
    if eval_config["device_num"] == "-1"
    else "cuda:{}".format(eval_config["device_num"])
)
...
tokenizer = BertTokenizerFast.from_pretrained(
    eval_config["bert_path"], add_special_tokens=False, do_lower_case=False
)
...
rel_extractor = get_tplinker_bert_model(
    eval_config["bert_path"], rel2id, hyper_parameters
)
...
def tplinker_predict():
    try:
        ...
        test_data = [{"id": id, "text": text}]

        data = preprocessor.split_into_short_samples(
            test_data,
            max_seq_len,
        ...
        return_result = {"errCode": 200, "data": result}
        return jsonify(return_result)
    except Exception:
        return_result = {"errCode": 200, "data": traceback.format_exc()}
        return jsonify(return_result)
