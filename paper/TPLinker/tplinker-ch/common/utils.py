    ...
    def split_into_short_samples(
        ...
        for sample in tqdm(sample_list, desc="Splitting into subtexts"):
            ...
            for start_ind in range(0, len(tokens), sliding_len):
                if (
                    encoder == "BERT"
                ):  # if use bert, do not split a word into two samples
                ...
                if data_type == "test":  # test set
                    ...
                    for rel in sample["relation_list"]:
                        ...
                        if (
                            ...
                            and subj_tok_span[1] <= end_ind
                            and obj_tok_span[0] >= start_ind
                            and obj_tok_span[1] <= end_ind
                        ):
                    ...
                    for ent in sample["entity_list"]:
                        ...
                        if tok_span[0] >= start_ind and tok_span[1] <= end_ind:
                            ...
                            new_ent["tok_span"] = [
                                tok_span[0] - start_ind,
                                tok_span[1] - start_ind,
                            ]
                    ...
                    if "event_list" in sample:
                        ...
                        for event in sample["event_list"]:
                            ...
                            if (
                                trigger_tok_span[1] > end_ind
                                or trigger_tok_span[0] < start_ind
                            ):
                                continue
                            ...
                            for arg in new_event["argument_list"]:
                                if (
                                    arg["tok_span"][0] >= start_ind
                                    and arg["tok_span"][1] <= end_ind
                                ):
    ...
    def clean_data_w_span(self, ori_data):
        ...
        for sample in tqdm(ori_data, desc="clean data w char spans"):
            ...
            for rel in sample["relation_list"]:
                ...
                if (
                    rel["subject"] not in text
                    or rel["subject"] != text[subj_char_span[0] : subj_char_span[1]]
                    or rel["object"] not in text
                    or rel["object"] != text[obj_char_span[0] : obj_char_span[1]]
                ):
                    rel["stake"] = 0
                    bad = True
    ...
    def _get_ent2char_spans(self, text, entities, ignore_subword_match=True):
        ...
        for ent in entities:
            ...
            for m in re.finditer(re.escape(target_ent), text_cp):
                if not ignore_subword_match and re.match(
                    "\d+", target_ent
                ):  # avoid matching a inner number of a number
                    if (
                        m.span()[0] - 1 >= 0
                        and re.match("\d", text_cp[m.span()[0] - 1])
                    ) or (
                        m.span()[1] < len(text_cp)
                        and re.match("\d", text_cp[m.span()[1]])
                    ):
                        continue
                # 英文数据集上减2
