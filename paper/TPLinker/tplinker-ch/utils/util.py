...
def get_tokenizer_bert(bert_path):
    tokenizer = BertTokenizerFast.from_pretrained(
        bert_path, add_special_tokens=False, do_lower_case=False
    )
...
def get_test_prf(pred_sample_list, gold_test_data, metrics, pattern="only_head_text"):
    ...
    for gold_n_pred in text_id2gold_n_pred.values():
        ...
        pred_rel_list = (
            gold_n_pred["pred_relation_list"]
            if "pred_relation_list" in gold_n_pred
            else []
        )
...
def predict(
    ...
    for batch_test_data in tqdm(test_dataloader, desc="Predicting"):
        ...
        (
            ...
            torch.argmax(batch_ent_shaking_outputs, dim=-1),
        ...
        for ind in range(len(sample_list)):
            ...
            ent_shaking_tag, head_rel_shaking_tag, tail_rel_shaking_tag = (
                batch_ent_shaking_tag[ind],
            ...
            rel_list = handshaking_tagger.decode_rel_fr_shaking_tag(
                text,
                ent_shaking_tag,
...
def get_model_training_scheduler(optimizer, batch_num, hyper_parameters):
    if hyper_parameters["scheduler"] == "CAWR":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, batch_num * rewarm_epoch_num, T_mult
        )
...
def bias_loss(device, weights=None):
    ...
    return lambda pred, target: cross_en(
        pred.view(-1, pred.size()[-1]), target.view(-1)
    )


def train_step(
    rel_extractor, batch_train_data, optimizer, loss_weights, loss_func, metrics, device
):
    (
        sample_list,
        batch_input_ids,
    ...
    loss = (
        w_ent * loss_func(ent_shaking_outputs, batch_ent_shaking_tag)
    ...
    ent_sample_acc = metrics.get_sample_accuracy(
        ent_shaking_outputs, batch_ent_shaking_tag
    )
...
def model_evaluate(dataloader, rel_extractor, metrics, logger, device, match_pattern):
    ...
    rel_extractor.eval()
    t_ep = time.time()
    total_ent_sample_acc, total_head_rel_sample_acc, total_tail_rel_sample_acc = (
        0.0,
        0.0,
        0.0,
    )
    ...
    for batch_ind, batch_valid_data in enumerate(tqdm(dataloader, desc="Validating")):
        (
            ...
            batch_valid_data, rel_extractor, metrics, device, match_pattern
        )
    ...
    rel_prf = metrics.get_prf_scores(
        total_rel_correct_num, total_rel_pred_num, total_rel_gold_num
    )
