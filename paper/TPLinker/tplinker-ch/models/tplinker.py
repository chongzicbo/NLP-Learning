...
class HandshakingTaggingScheme(object):
    ...
    def get_spots(self, sample):
        ...
        for rel in sample["relation_list"]:
            ...
            ent_matrix_spots.append(
                (subj_tok_span[0], subj_tok_span[1] - 1, self.tag2id_ent["ENT-H2T"])
            )
            ent_matrix_spots.append(
                (obj_tok_span[0], obj_tok_span[1] - 1, self.tag2id_ent["ENT-H2T"])
            )

            if subj_tok_span[0] <= obj_tok_span[0]:
                head_rel_matrix_spots.append(
                    (
                        self.rel2id[rel["predicate"]],
                        subj_tok_span[0],
                        obj_tok_span[0],
    ...
    def spots2shaking_tag4batch(self, batch_spots):
        ...
        """
        shaking_seq_len = self.matrix_size * (self.matrix_size + 1) // 2
        batch_shaking_seq_tag = torch.zeros(
            len(batch_spots), len(self.rel2id), shaking_seq_len
        ).long()
        for batch_id, spots in enumerate(batch_spots):
            for sp in spots:
                shaking_ind = self.matrix_ind2shaking_ind[sp[1]][sp[2]]
    ...
    def decode_rel_fr_shaking_tag(
        ...
        for sp in ent_matrix_spots:
            ...
            head_key = sp[
                0
            ]  # take head as the key to entity list start with the head token
            ...
            for subj in subj_list:
                for obj in obj_list:
                    tail_rel_memory = "{}-{}-{}".format(
                        rel_id, subj["tok_span"][1] - 1, obj["tok_span"][1] - 1
                    )
...
class DataMaker4Bert:
    ...
    def generate_batch(self, batch_data, data_type="train"):
        ...
        if data_type != "test":
            batch_ent_shaking_tag = (
                self.handshaking_tagger.sharing_spots2shaking_tag4batch(ent_spots_list)
            )
            batch_head_rel_shaking_tag = (
                self.handshaking_tagger.spots2shaking_tag4batch(head_rel_spots_list)
            )
            batch_tail_rel_shaking_tag = (
                self.handshaking_tagger.spots2shaking_tag4batch(tail_rel_spots_list)
            )
...
class TPLinkerBert(nn.Module):
    def __init__(
        ...
        self.handshaking_kernel = HandshakingKernel(
            hidden_size, shaking_type, inner_enc_type
        )
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        if self.dist_emb_size != -1:
            ...
            if self.dist_embbedings is None:
                dist_emb = torch.zeros([self.dist_emb_size, hidden_size]).to(
                ...
                for d in range(self.dist_emb_size):
                    for i in range(hidden_size):
                        if i % 2 == 0:
                            ...
                            dist_emb[d][i] = math.cos(
                                d / 10000 ** ((i - 1) / hidden_size)
                            )
            ...
            if self.ent_add_dist:
                shaking_hiddens4ent = shaking_hiddens + self.dist_embbedings[
                    None, :, :
                ].repeat(shaking_hiddens.size()[0], 1, 1)
            if self.rel_add_dist:
                shaking_hiddens4rel = shaking_hiddens + self.dist_embbedings[
                    None, :, :
                ].repeat(shaking_hiddens.size()[0], 1, 1)
...
class MetricsCalculator:
    ...
    def get_rel_cpg(
        ...
        batch_pred_ent_shaking_tag = torch.argmax(
            batch_pred_ent_shaking_outputs, dim=-1
        )
        batch_pred_head_rel_shaking_tag = torch.argmax(
            batch_pred_head_rel_shaking_outputs, dim=-1
        )
        batch_pred_tail_rel_shaking_tag = torch.argmax(
            batch_pred_tail_rel_shaking_outputs, dim=-1
        )
