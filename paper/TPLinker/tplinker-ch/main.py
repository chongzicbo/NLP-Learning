...
def main():
    ...
    device = (
        ...
        else "cuda:{}".format(train_config["device_num"])
    )
    ...
    tokenizer = BertTokenizerFast.from_pretrained(
        train_config["bert_path"], add_special_tokens=False, do_lower_case=False
    )
    ...
    train_data_path = os.path.join(
        train_config["data_home"], train_config["train_data"]
    )
    valid_data_path = os.path.join(
        train_config["data_home"], train_config["valid_data"]
    )
    ...
    if max_tok_num > hyper_parameters["max_seq_len"]:
        ...
        logger.info(
            "train: {}".format(len(train_data)), "valid: {}".format(len(valid_data))
        )
    # 更新max token num
    max_seq_len = min(max_tok_num, hyper_parameters["max_seq_len"])
    # 定义handshaking
    handshaking_tagger = HandshakingTaggingScheme(
        rel2id=rel2id, max_seq_len=max_seq_len
    )
    ...
    # get tplinker model
    rel_extractor = get_tplinker_bert_model(
        train_config["bert_path"], rel2id, hyper_parameters
    )
    ...
    # get scheduler
    scheduler = get_model_training_scheduler(
        optimizer, len(train_dataloader), hyper_parameters
    )
    logger.info("开始训练")
    best_f1 = 0.0
    for epoch in range(hyper_parameters["epochs"]):
        ...
        (
            total_loss,
            total_ent_sample_acc,
            total_head_rel_sample_acc,
            total_tail_rel_sample_acc,
        ) = (0.0, 0.0, 0.0, 0.0)
        for batch_ind, batch_train_data in enumerate(train_dataloader):
            ...
            total_steps = (
                hyper_parameters["loss_weight_recover_steps"] + 1
            )  # + 1 avoid division by zero error
            current_step = steps_per_ep * epoch + batch_ind
            w_ent = max(1 / z + 1 - current_step / total_steps, 1 / z)
            w_rel = min(
                (len(rel2id) / z) * current_step / total_steps, (len(rel2id) / z)
            )
            ...
            batch_print_format = (
                "\rrun_name: {}, Epoch: {}/{}, batch: {}/{}, train_loss: {}, "
