...
class TaggerNERDataset(Dataset):
    ...
    def __init__(
        ...
        self.label2idx = {
            ...
            for label_idx, label_item in enumerate(get_labels(dataset_signature))
        }
    ...
    def __getitem__(self, item):
        ...
        for token_item, label_item in zip(token_lst, label_lst):
            tmp_token_lst = self.tokenizer.encode(
                token_item, add_special_tokens=False, return_token_type_ids=None
            )
            if len(tmp_token_lst) == 1:
                ...
                tmp_label_lst = [label_item] + [
                    -100 for idx in range((len_wordpiece - 1))
                ]
        ...
        is_wordpiece_mask = [
            1 if label_item != -100 else -100 for label_item in wordpiece_label_lst
        ]
        wordpiece_label_idx_lst = [
            self.label2idx[label_item] if label_item != -100 else -100
            for label_item in wordpiece_label_lst
        ]

        return [
            torch.tensor(wordpiece_token_lst, dtype=torch.long),
