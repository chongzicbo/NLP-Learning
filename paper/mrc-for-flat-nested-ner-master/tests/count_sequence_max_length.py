...
class OntoNotesDataConfig:
    def __init__(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.model_path, use_fast=False, tokenize_chinese_chars=True
        )
...
class EnglishCoNLLDataConfig:
    def __init__(self):
        ...
        if "uncased" in self.model_path:
            ...
            self.do_lower_case = False
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.model_path, use_fast=False, do_lower_case=self.do_lower_case
        )
        self.max_length = 512
        self.is_chinese = False
        self.threshold = 275
...
def count_max_length(data_sign):
    ...
    for prefix in ["test", "train", "dev"]:
        ...
        data_file_path = os.path.join(
            data_config.data_dir, f"{prefix}.{data_config.data_file_suffix}"
        )
