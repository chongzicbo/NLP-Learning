...
def find_data_instance(file_path, search_string):
    ...
    for sentence_lst, label_lst in zip(
        sentence_collections, sentence_label_collections
    ):
...
def find_illegal_entity(
    ...
    if len(start_positions) != 0:
        for start_index, end_index in zip(start_positions, end_positions):
            if is_chinese:
                ...
                answer_text_span = " ".join(
                    context.split(" ")[start_index : end_index + 1]
                )
            new_start, new_end = _improve_answer_span(
                query_context_tokens["input_ids"],
                first_sep_token,
                end_sep_token,
                tokenizer,
                answer_text_span,
    ...
    new_start_positions = [
        start_pos for start_pos in new_start_positions if start_pos < 500000
    ]
    ...
    for start_item, end_item in zip(new_start_positions, new_end_positions):
        print(
            tokenizer.convert_ids_to_tokens(
                query_context_tokens["input_ids"][start_item : end_item + 1]
            )
        )
...
if __name__ == "__main__":
    ...
    find_illegal_entity(
        query, context_tokens, labels, model_path, is_chinese=False, do_lower_case=False
    )
