...
class BertSequenceLabeling(pl.LightningModule):
    def __init__(self, args: argparse.Namespace):
        ...
        if isinstance(args, argparse.Namespace):
            ...
            logging.basicConfig(
                format=format,
                filename=os.path.join(self.args.output_dir, "eval_result_log.txt"),
                level=logging.INFO,
            )
        else:
            # eval mode
            ...
            logging.basicConfig(
                format=format,
                filename=os.path.join(self.args.output_dir, "eval_test.txt"),
                level=logging.INFO,
            )
        ...
        self.task_idx2label = {
            ...
            for label_idx, label_item in enumerate(get_labels(self.args.data_sign))
        }
        bert_config = BertTaggerConfig.from_pretrained(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            args.bert_config_dir, use_fast=False, do_lower_case=args.do_lowercase
        )
        self.model = BertTagger.from_pretrained(
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            "--train_batch_size", type=int, default=8, help="batch size"
        )
        ...
        parser.add_argument(
            "--classifier_intermediate_hidden_size", type=int, default=1024
        )
        parser.add_argument("--chinese", action="store_true", help="is chinese dataset")
        parser.add_argument(
            "--optimizer",
            choices=["adamw", "torch.adam"],
            default="adamw",
            help="optimizer type",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="the path for saving intermediate model checkpoints.",
        )
        parser.add_argument(
            "--lr_scheduler", type=str, default="linear_decay", help="lr scheduler"
        )
        parser.add_argument(
            "--data_sign",
            type=str,
            default="en_conll03",
            help="data signature for the dataset.",
        )
        parser.add_argument(
            "--polydecay_ratio",
            type=float,
            default=4,
            help="ratio for polydecay learing rate scheduler.",
        )
        parser.add_argument(
            "--do_lowercase",
            action="store_true",
    ...
    def configure_optimizers(self):
        ...
        if self.args.lr_scheduler == "onecycle":
            ...
            scheduler = get_polynomial_decay_schedule_with_warmup(
                optimizer, warmup_steps, t_total, lr_end=lr_mini
            )
        else:
            raise ValueError
    ...
    def validation_step(self, batch, batch_idx):
        ...
        sequence_pred_lst = transform_predictions_to_labels(
            logits.view(batch_size, -1, len(self.task_labels)),
            is_wordpiece_mask,
            self.task_idx2label,
            input_type="logit",
        )
        sequence_gold_lst = transform_predictions_to_labels(
            sequence_labels, is_wordpiece_mask, self.task_idx2label, input_type="label"
        )
    ...
    def validation_epoch_end(self, outputs):
        ...
        all_counts = (
            torch.stack([x[f"span_f1_stats"] for x in outputs]).view(-1, 3).sum(0)
        )
    ...
    def test_epoch_end(self, outputs) -> Dict[str, Dict[str, Tensor]]:
        ...
        self.result_logger.info(
            f"EVAL INFO -> test_f1 is: {span_f1}, test_precision is: {span_precision}, test_recall is: {span_recall}"
        )
    ...
    def get_dataloader(self, prefix="train", limit: int = None) -> DataLoader:
        ...
        dataset = TaggerNERDataset(
            data_path,
            self.tokenizer,
            self.args.data_sign,
            max_length=self.args.max_length,
            is_chinese=self.args.chinese,
            pad_to_maxlen=False,
        )
...
def find_best_checkpoint_on_dev(
    ...
    for checkpoint_info_line in checkpoint_info_lines:
        current_f1 = float(
            re.findall(F1_PATTERN, checkpoint_info_line)[0]
            .replace("span_f1 reached ", "")
            .replace(" (best", "")
        )
        current_ckpt = (
            re.findall(CKPT_PATTERN, checkpoint_info_line)[0]
            .replace("saving model to ", "")
            .replace(" as top", "")
        )
