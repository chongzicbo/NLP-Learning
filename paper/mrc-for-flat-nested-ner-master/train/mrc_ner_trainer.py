...
class BertLabeling(pl.LightningModule):
        ...
        if isinstance(args, argparse.Namespace):
            ...
            logging.basicConfig(
                ...
                filename=os.path.join(
                    self.args.default_root_dir, "eval_result_log.txt"
                ),
                level=logging.INFO,
            )
        else:
    ...
    def configure_optimizers(self):
        ...
        if self.args.lr_scheduler == "onecycle":
            ...
            scheduler = get_polynomial_decay_schedule_with_warmup(
                optimizer, self.args.warmup_steps, t_total, lr_end=lr_mini
            )
        else:
            raise ValueError
    ...
    def compute_loss(
        ...
        if self.span_loss_candidates == "all":
            ...
            if self.span_loss_candidates == "gold":
                ...
                random_matrix = torch.bernoulli(
                    random_matrix, generator=data_generator
                ).long()
        ...
        match_loss = self.bce_loss(
            span_logits.view(batch_size, -1), match_labels.view(batch_size, -1).float()
        )
