...
from metrics.functional.tagger_span_f1 import (
    get_entity_from_bmes_lst,
    transform_predictions_to_labels,
)
...
def get_dataloader(config, data_prefix="test"):
    ...
    data_tokenizer = AutoTokenizer.from_pretrained(
        config.bert_dir, use_fast=False, do_lower_case=config.do_lowercase
    )
...
def main():
    ...
    task_idx2label = {
        label_idx: label_item for label_idx, label_item in enumerate(entity_label_lst)
    }
    ...
    for batch in data_loader:
        ...
        logits = trained_tagger_ner_model.model(
            token_input_ids,
        ...
        sequence_pred_lst = transform_predictions_to_labels(
            logits.view(batch_size, -1, len(entity_label_lst)),
            is_wordpiece_mask,
            task_idx2label,
            input_type="logit",
        )
        ...
        readable_input_str = data_tokenizer.decode(
            batch_subtokens_idx_lst, skip_special_tokens=True
        )

        batch_entity_lst = [
            get_entity_from_bmes_lst(label_lst_item)
            for label_lst_item in sequence_pred_lst
        ]
