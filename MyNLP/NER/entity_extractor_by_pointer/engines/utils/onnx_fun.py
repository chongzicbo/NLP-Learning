...
class ONNXAdds:
    ...
    def tril_onnx(self, inputs: torch.FloatTensor,
                  diagonal: Optional[int] = 0) -> torch.FloatTensor:

        inputs_wrapper = []
        if len(inputs.shape) == 2:
            return self.tril_onnx2(inputs, diagonal)
        elif len(inputs.shape) == 3:
            for i in range(inputs.size(0)):
                tmp_inputs = torch.clone(inputs[i])
                inputs_wrapper.append(self.tril_onnx2(tmp_inputs).tolist())
        elif len(inputs.shape) == 4:
            inputs_wrapper_inner = []
            for i in range(inputs.size(1)):
                tmp_inputs = torch.clone(inputs[0][i])
                inputs_wrapper_inner.append(self.tril_onnx2(tmp_inputs).tolist())
