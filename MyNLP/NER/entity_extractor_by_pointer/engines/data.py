    ...
    def prepare_data(self, data):
        ...
        for item in data:
            ...
            if self.configs['model_type'] == 'bp':
                label_vector = np.zeros((len(token_ids), len(self.categories), 2))  # 转换为两个二分类问题
            else:
                label_vector = np.zeros((self.num_labels, len(token_ids), len(token_ids)))

            for entity in item.get('entities'):
                start_idx = entity['start_idx']  # 实体开始位置索引
                end_idx = entity['end_idx']  # 实体终止位置
                type_class = entity['type']  # 实体类别
                # tokenizer之后，每个token对应的wordpiece的位置映射，中文不影响，英文单词进行wordpiece后可能拆分为多个
                ...
                if start_idx in start_mapping and end_idx in end_mapping:
                    class_id = self.categories[type_class]  # 实体类别id
                    ...
                    if self.configs['model_type'] == 'bp':
                        label_vector[start_in_tokens, class_id, 0] = 1  # 实体开始位置，对应的实体类别设置为1，二分类问题
                        label_vector[end_in_tokens, class_id, 1] = 1  # 实体结束位置，对应的实体类别设置为1
                    else:
        ...
        label_vectors = torch.tensor(np.array(label_vectors))  # [batch_size,seq_len,num_labels,2]
