...
class LongDocClassifier:
    ...
    def _input_preparation(
        self, dataset="iflytek", test_name="test", preprocess_text_fn=None
    ):
    ...
    def _construct_input_spec(self):
        ...
        self._input_spec = [
            paddle.static.InputSpec(
                shape=[B, T, 1], dtype="int64", name="input_ids"
            ),  # input_ids
            paddle.static.InputSpec(
                shape=[N, B, M, H], dtype="float32", name="memories"
            ),  # memories
            paddle.static.InputSpec(
                shape=[B, T, 1], dtype="int64", name="token_type_ids"
            ),  # token_type_ids
            paddle.static.InputSpec(
                shape=[B, 2 * T + M, 1], dtype="int64", name="position_ids"
            ),  # position_ids
            paddle.static.InputSpec(
                shape=[B, T, 1], dtype="float32", name="attn_mask"
            ),  # attn_mask
        ]
    ...
    def run_model(self, saved_path):
        ...
        if not self.static_mode:
            self.input_handles, self.output_handle, self.predictor = (
                None,
                None,
                self._model,
            )
