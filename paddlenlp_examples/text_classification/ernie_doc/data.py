...
def get_related_pos(insts, seq_len, memory_len=128):
    ...
    r_position = [
        list(range(beg - 1, seq_len - 1, -1)) + list(range(0, seq_len))
    ...
    if return_input_mask:
        ...
        if final_cls:
            input_mask_data = np.array(
                [
                    [1] * len(inst[:-1]) + [0] * (max_len - len(inst)) + [1]
                    for inst in insts
                ]
            )
        else:
...
class ClassifierIterator(object):
    def __init__(
        ...
        self.mode = mode
        self.shuffle = True if mode == "train.json" else False
    ...
    def _convert_to_features(self, example, qid):
        ...
        for doc_span_index, doc_span in enumerate(doc_spans):
            tokens = (
                tokens_a[doc_span.start : doc_span.start + doc_span.length]
                + ["[SEP]"]
                + ["[CLS]"]
            )
    ...
    def _pad_batch_records(self, batch_records, gather_idx=[]):
        ...
        if gather_idx:
            ...
            batch_gather_idx = (
                np.array(list(range(len(batch_records))))
                .astype("int64")
                .reshape([-1, 1])
            )
        ...
        padded_position_ids = get_related_pos(
            padded_token_ids, self.max_seq_length, self.memory_len
        )
    ...
    def _create_instances(self):
        ...
        if self.mode != "train.json":
            if self._cnt_list(pre_batch_list):
                pre_batch_list += [
                    []
                    for _ in range(
                        self.batch_size * self.trainer_num
    ...
    def __call__(self):
        ...
        for batch_records in self._create_instances():
            if curr_id == self.trainer_id or self.mode != "train.json":
                yield batch_records
...
class MRCIterator(ClassifierIterator):
    ...
    def _convert_example_to_feature(self, examples):
        ...
        self.features_all = []
        unique_id = 1000
        is_training = self.mode == "train.json"
    ...
    def _pad_batch_records(self, batch_records, gather_idx=[]):
        ...
        if self.mode == "train.json":
            ...
            batch_start_position = (
                np.array(batch_start_position).astype("int64").reshape([-1, 1])
            )
            batch_end_position = (
                np.array(batch_end_position).astype("int64").reshape([-1, 1])
            )
        else:
...
class MCQIterator(MRCIterator):
        ...
        for ex_index, example in enumerate(examples):
            ...
            question_choice_pairs = [
                self._truncate_seq_pair(
                    question_tokens, choice_tokens, self.max_query_length - 2
                )
    ...
    def _pad_batch_records(self, batch_records, gather_idx=[]):
        batch_token_ids = [
            [record.src_ids for record in records] for records in batch_records
        ]
        if batch_records[0][0].label is not None:
            batch_labels = [
                [record.label for record in records] for records in batch_records
            ]
            batch_labels = np.array(batch_labels).astype("int64").reshape([-1, 1])
        else:
            batch_labels = np.array([]).astype("int64").reshape([-1, 1])
        ...
        batch_task_ids = [
            [record.segment_ids for record in records] for records in batch_records
        ]
        ...
        batch_padded_token_ids = (
            np.array(batch_padded_token_ids)
            .astype("int64")
            .reshape([batch_size * self.choice_num, -1, 1])
        )
        batch_padded_position_ids = (
            np.array(batch_padded_position_ids)
            .astype("int64")
            .reshape([batch_size * self.choice_num, -1, 1])
        )
        batch_padded_task_ids = (
            np.array(batch_padded_task_ids)
            .astype("int64")
            .reshape([batch_size * self.choice_num, -1, 1])
        )
        batch_input_mask = (
            np.array(batch_input_mask)
            .astype("float32")
            .reshape([batch_size * self.choice_num, -1, 1])
        )
...
class SemanticMatchingIterator(MRCIterator):
    def _convert_qa_to_examples(self):
        ...
        for qid, qa in enumerate(self.dataset):
            ...
            example = Example(
                qid=qid, text_a=text_a, text_b=text_b, text_c=text_c, label=qa["label"]
            )
            examples += [example]
        return examples
    ...
    def _convert_example_to_feature(self, examples):
        ...
        for ex_index, example in enumerate(examples):
            ...
            a_len, b_len, c_len = list(
                map(lambda x: len(x), [text_a_tokens, text_b_tokens, text_c_tokens])
            )
...
class SequenceLabelingIterator(ClassifierIterator):
    ...
    def _convert_to_features(self, example, qid):
        ...
        for doc_span_index, doc_span in enumerate(doc_spans):
            curr_tokens = (
                ["[CLS]"]
                + tokens[doc_span.start : doc_span.start + doc_span.length]
                + ["[SEP]"]
            )
            token_ids = self.tokenizer.convert_tokens_to_ids(curr_tokens)
            label = (
                [self.no_entity_id]
                + label[doc_span.start : doc_span.start + doc_span.length]
                + [self.no_entity_id]
            )

            features.append(
                Feature(src_ids=token_ids, label_ids=label, qid=qid, cal_loss=1)
            )
