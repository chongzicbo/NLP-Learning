...
class MultiHeadAttention(nn.Layer):
    def __init__(
        ...
        self.q_proj = nn.Linear(
            d_model, d_key * n_head, weight_attr=weight_attr, bias_attr=bias_attr
        )
        self.k_proj = nn.Linear(
            d_model, d_key * n_head, weight_attr=weight_attr, bias_attr=bias_attr
        )
        self.v_proj = nn.Linear(
            d_model, d_value * n_head, weight_attr=weight_attr, bias_attr=bias_attr
        )
        ...
        self.dropout = (
            nn.Dropout(dropout_rate, mode="upscale_in_train") if dropout_rate else None
        )
    ...
    def forward(self, queries, keys, values, rel_pos, rel_task, memory, attn_mask):
        ...
        q_w, q_r, q_t = list(
            map(
                lambda x: q + x.unsqueeze([0, 1]),
                [self.r_w_bias, self.r_r_bias, self.r_t_bias],
            )
        )
        q_w, q_r, q_t = list(
            map(
                lambda x: self._split_heads(x, self.d_model, self.n_head),
                [q_w, q_r, q_t],
            )
        )
        k, v, r, t = list(
            map(lambda x: self._split_heads(x, self.d_model, self.n_head), [k, v, r, t])
        )
        ctx_multiheads = self._scaled_dot_product_attention(
            [q_w, q_r, q_t], k, v, r, t, attn_mask
        )
...
class ErnieDocEncoderLayer(nn.Layer):
    def __init__(
        ...
        if not rel_pos_params_sharing:
            r_w_bias, r_r_bias, r_t_bias = list(
                map(
                    lambda x: self.create_parameter(
    ...
    def forward(self, enc_input, memory, rel_pos, rel_task, attn_mask):
        ...
        attn_output = self.attn(
            enc_input, enc_input, enc_input, rel_pos, rel_task, memory, attn_mask
        )
    ...
    def forward(self, enc_input, memories, rel_pos, rel_task, attn_mask):
        ...
        for _, encoder_layer in enumerate(self.layers):
            ...
            enc_input = encoder_layer(
                enc_input, memories[0], rel_pos, rel_task, attn_mask
            )
            if new_mem is None:
...
class ErnieDocModel(ErnieDocPretrainedModel):
    ...
    def _create_n_head_attn_mask(self, attn_mask, batch_size):
        ...
        data_mask = paddle.concat(
            [
                paddle.ones(
                    shape=[batch_size, self.memory_len, 1], dtype=attn_mask.dtype
                ),
                attn_mask,
            ],
            axis=1,
