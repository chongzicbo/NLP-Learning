...
parser.add_argument("--device", choices=["cpu", "gpu", "xpu"], default="%s" % device,
                    help="Select which device to train.json model, defaults to gpu.")
parser.add_argument("--data_path", type=str,
                    default="E:\\opensource_data\\分类\\jigsaw-toxic-comment-classification-challenge\\",
...
def do_train():
    ...
    batchify_fn = lambda samples, fn=Tuple(
        Pad(axis=0, pad_val=tokenizer.pad_token_id),  # 针对token_ids
        Pad(axis=0, pad_val=tokenizer.pad_token_type_id),  # 针对segment_ids
        Stack(dtype="float32")  # 针对label_id
    ...
    model = MultiLabelClassifier(pretrained_model, num_labels=2)
    ...
    for epoch in range(1, args.epochs + 1):
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            if global_step % 10 == 0 and rank == 0:
                print("global step %d,epoch:%d,batch:%d,loss:%.5f,auc:%.5f,f1 score:%.5f,speed:%.2f step/2" % (
