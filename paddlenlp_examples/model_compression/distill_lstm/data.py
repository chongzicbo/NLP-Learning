...
def apply_data_augmentation_for_cn(
    ...
    for example in data:
        ...
        for _ in range(n_iter):
            ...
            # 2. N-gram sampling
            lstm_tokens, bert_tokens = ngram_sampling(
                lstm_tokens, bert_tokens, p_ng, ngram_range
            )
...
def create_data_loader_for_small_model(
    ...
    if task_name == "chnsenticorp":
        train_ds, dev_ds = load_dataset(task_name, splits=["train.json", "dev"])
    else:
        train_ds, dev_ds = load_dataset("glue", task_name, splits=["train.json", "dev"])
    if task_name == "chnsenticorp":
