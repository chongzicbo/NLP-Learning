...
class BiLSTM(nn.Layer):
    def __init__(
        ...
        self.lstm = nn.LSTM(
            embed_dim, hidden_size, num_layers, "bidirectional", dropout=dropout_prob
        )
        self.fc = nn.Linear(
            hidden_size * 2,
            hidden_size,
    ...
    def forward(self, x_1, seq_len_1, x_2=None, seq_len_2=None, *inputs, **kwargs):
        ...
        if x_2 is not None:
            ...
            out = paddle.concat(
                x=[out_1, out_1 + out_2, paddle.abs(out_1 - out_2)], axis=1
            )
        ...
        for i, batch in enumerate(train_data_loader):
            ...
            if global_step % args.log_freq == 0:
                with paddle.no_grad():
                    ...
                    acc = evaluate(
                        args.task_name, model, loss_fct, metric, dev_data_loader
                    )
            ...
            if global_step % args.save_steps == 0:
                paddle.save(
                    model.state_dict(),
                    os.path.join(
                        args.output_dir, "step_" + str(global_step) + ".pdparams"
                    ),
                )
                paddle.save(
                    optimizer.state_dict(),
                    os.path.join(
                        args.output_dir, "step_" + str(global_step) + ".pdopt"
                    ),
                )
