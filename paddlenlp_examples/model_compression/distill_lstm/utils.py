...
def convert_example_for_lstm(
    example, task_name, vocab, is_tokenized=False, max_seq_length=128, is_test=False
):
    input_ids = []
    ...
    if not is_test:
        label = (
            ...
            if task_name == "chnsenticorp"
            else np.array(example["label"], dtype="int64")
        )
        return input_ids, valid_length, label


def convert_pair_example(
    example, task_name, vocab, is_tokenized=True, max_seq_length=128, is_test=False
):
