...
@author: chengbo
@software: PyCharm
@file: general_distill.py
@time: 2022/6/23 16:35
"""
...
def create_pretraining_dataset(input_file, args, worker_init, tokenizer):
    train_data = PretrainingDataset(
        input_file=input_file, tokenizer=tokenizer, max_seq_length=args.max_seq_length
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for f_id in range(f_start_id, len(files)):
            ...
            dataset_future = pool.submit(
                create_pretraining_dataset, data_file, args, worker_init, tokenizer
            )
            ...
            for step, batch in enumerate(train_data_loader):
                ...
                loss_qr = calc_minilm_loss(
                    kl_loss_func, q_s, q_t, attention_mask, args.num_relation_heads
                )
                del q_t, q_s
                # K-K relation
                k_t, k_s = teacher.outputs.k, student.outputs.k
                loss_kr = calc_minilm_loss(
                    kl_loss_func, k_s, k_t, attention_mask, args.num_relation_heads
                )
                del k_t, k_s
                # V-V relation
                v_t, v_s = teacher.outputs.v, student.outputs.v
                loss_vr = calc_minilm_loss(
                ...
                if (
                    ...
                    if paddle.distributed.get_rank() == 0:
                        output_dir = os.path.join(
                            args.output_dir, "model_%d" % global_step
                        )
