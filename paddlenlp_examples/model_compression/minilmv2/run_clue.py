...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the task to train.json selected in the list: "
    ...
    parser.add_argument(
        "--max_grad_norm", default=1.0, type=float, help="The max value of grad norm."
    )
...
def convert_example(example, tokenizer, label_list, max_seq_length=512, is_test=False):
    ...
    if "sentence" in example:
        ...
        # print(text)
        assert (
            text[pronoun_idx : (pronoun_idx + len(pronoun))] == pronoun
        ), "pronoun: {}".format(pronoun)
        assert text[query_idx : (query_idx + len(query))] == query, "query: {}".format(
            query
        )
...
def do_train(args):
    ...
    train_ds = load_dataset("clue", args.task_name, splits="train.json")
