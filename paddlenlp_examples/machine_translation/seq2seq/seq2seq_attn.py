...
@author: chengbo
@software: PyCharm
@file: seq2seq_attn.py
@time: 2022/6/26 14:51
"""
...
class Seq2SeqDecoder(nn.Layer):
    def __init__(
        ...
        self.lstm_attention = nn.RNN(
            Seq2SeqDecoderCell(num_layers, embed_dim, hidden_size, dropout_prob),
            is_reverse=False,
            time_major=False,
        )
    ...
    def forward(
        self, trg, decoder_initial_states, encoder_output, encoder_padding_mask
    ):
...
class Seq2SeqAttnModel(nn.Layer):
    def __init__(
        ...
        self.encoder = Seq2SeqEncoder(
            src_vocab_size, embed_dim, hidden_size, num_layers, dropout_prob, init_scale
        )
        self.decoder = Seq2SeqDecoder(
            trg_vocab_size, embed_dim, hidden_size, num_layers, dropout_prob, init_scale
        )
    ...
    def forward(self, src, src_length, trg):
        ...
        predict = self.decoder(
            trg, decoder_initial_states, encoder_output, encoder_padding_mask
        )
        return predict
