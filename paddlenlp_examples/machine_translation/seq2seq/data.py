...
def create_train_loader(args):
    ...
    train_ds, dev_ds = load_dataset("iwslt15", splits=("train.json", "dev"))
    ...
    train_batch_sampler = (
        SamplerHelper(train_ds)
        .shuffle()
        .sort(key=key, buffer_size=batch_size * 20)
        .batch(batch_size=batch_size)
    )

    dev_batch_sampler = (
        SamplerHelper(dev_ds)
        .sort(key=key, buffer_size=batch_size * 20)
        .batch(batch_size=batch_size)
    )
...
def prepare_infer_input(insts, bos_id, eos_id, pad_id):
    insts = [
        ([bos_id] + inst[0] + [eos_id], [bos_id] + inst[1] + [eos_id]) for inst in insts
    ]
