...
@author: chengbo
@software: PyCharm
@file: run_summarization.py
@time: 2022/6/14 16:52
"""
...
def do_train(args):
    ...
    logger.info("Loading train.json and dev dataset: %s" % args.dataset_name)
    train_set, dev_set = load_dataset(args.dataset_name, splits=["train", "dev"])
    logger.info("Loaded train.json and dev dataset: %s" % args.dataset_name)
    ...
    if paddle.distributed.get_rank() == 0:
        output_dir = os.path.join(
            args.output_dir, "bart_model_final_%d.pdparams" % global_step
        )
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        # Need better way to get inner model of DataParallel
