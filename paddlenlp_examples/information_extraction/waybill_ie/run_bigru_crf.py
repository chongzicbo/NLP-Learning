...
# yapf: disable
parser.add_argument("--save_dir", default='./bigru_crf_ckpt', type=str,
...
parser.add_argument("--device", default="gpu", type=str, choices=["cpu", "gpu"],
                    help="The device to select to train.json the model, is must be cpu/gpu.")
parser.add_argument("--data_dir", default='./waybill_ie/data', type=str,
...
if __name__ == '__main__':
    ...
    train_ds, dev_ds, test_ds = load_dataset(
        datafiles=(os.path.join(args.data_dir, 'train.json.txt'),
    ...
    batchify_fn = lambda samples, fn=Tuple(Pad(axis=0, pad_val=word_vocab.get("OOV", 0), dtype="int32"),
                                           Stack(dtype="int64"),
                                           Pad(axis=0, pad_val=label_vocab.get("O", 0), dtype="int64")): fn(samples)
    ...
    for epoch in range(args.epochs):
        ...
        paddle.save(model.state_dict(), os.path.join(args.save_dir, "model_%d" % step))
