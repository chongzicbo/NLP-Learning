...
parser.add_argument("--device", default="gpu", type=str, choices=["cpu", "gpu", "xpu"] ,help="The device to select to train.json the model, is must be cpu/gpu/xpu.")
# yapf: enable
...
def do_predict(args):
    ...
    train_examples, predict_examples = load_dataset(
        "msra_ner", split=("train.json", "test")
    )
    ...
    predict_ds = predict_examples.map(
