...
parser.add_argument("--model_name_or_path", default=None, type=str, required=True,
                    help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(
                        sum([list(classes[-1].pretrained_init_configuration.keys()) for classes in
                             MODEL_CLASSES.values()], [])), )
parser.add_argument("--dataset", default="msra_ner", type=str, choices=["msra_ner", "peoples_daily_ner"],
...
def evaluate(model, loss_fct, metric, data_loader, label_num, mode="valid"):
    ...
    for batch in data_loader:
        logits = mode(batch["input_ids"], batch["token_type_ids"])
...
def do_train(args):
    ...
    train_ds = raw_datasets["train.json"]
