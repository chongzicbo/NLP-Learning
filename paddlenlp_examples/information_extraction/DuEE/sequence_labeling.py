...
parser.add_argument("--learning_rate", type=float, default=5e-5, help="Learning rate used to train.json with warmup.")
parser.add_argument("--tag_path", type=str, default=None, help="tag set path")
parser.add_argument("--train_data", type=str, default=None, help="train.json data")
...
parser.add_argument("--do_train", type=ast.literal_eval, default=True, help="do train.json")
    ...
    for epoch in range(args.num_epoch):
        for idx, (input_ids, token_type_ids, seq_lens,
            ...
            if step > 0 and step % args.skip_step == 0 and rank == 0:
                print(
                    f'train.json epoch: {epoch} - step: {step} (total: {num_training_steps}) - loss: {loss_item:.6f}'
                )
