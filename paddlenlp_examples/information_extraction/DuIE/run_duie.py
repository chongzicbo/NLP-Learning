...
parser.add_argument("--do_train", action='store_true', default=False, help="do train.json")
...
def evaluate(model, criterion, data_loader, file_path, mode):
    ...
    for batch in tqdm(data_loader, total=len(data_loader)):
        ...
        mask = (
            (input_ids != 0).logical_and((input_ids != 1)).logical_and((input_ids != 2))
        )
    ...
    predict_zipfile_path = write_prediction_results(
        formatted_outputs, predict_file_path
    )
...
def do_train():
    ...
    test_dataset = DuIEDataset.from_file(
        eval_file_path, tokenizer, args.max_seq_length, True
    )
    ...
    lr_scheduler = LinearDecayWithWarmup(
        args.learning_rate, num_training_steps, args.warmup_ratio
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_data_loader):
            ...
            mask = (
                (input_ids != 0)
                .logical_and((input_ids != 1))
                .logical_and((input_ids != 2))
            )
            loss = criterion(logits, labels, mask)
            loss.backward()
            optimizer.step()
            ...
            if global_step % save_steps == 0 and rank == 0:
                ...
                print(
                    "saving checkpoing model_%d.pdparams to %s "
                ...
                model.train()  # back to train.json mode
...
def do_predict():
    ...
    # Loads dataset.
    test_dataset = DuIEDataset.from_file(
        args.predict_data_file, tokenizer, args.max_seq_length, True
    )
    collator = DataCollator()
    test_batch_sampler = paddle.io.BatchSampler(
        test_dataset, batch_size=args.batch_size, shuffle=False, drop_last=True
    ...
    if not (
        os.path.exists(args.init_checkpoint) and os.path.isfile(args.init_checkpoint)
    ):
        sys.exit(
            "wrong directory: init checkpoints {} not exist".format(
