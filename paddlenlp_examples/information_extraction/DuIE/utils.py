...
def find_entity(
    text_raw, id_, predictions, tok_to_orig_start_index, tok_to_orig_end_index
):
    """
...
def decoding(
    ...
    for i, (
        ...
        logits = logits[
            1 : seq_len + 1
        ]  # slice between [CLS] and [SEP] to get valid logits
        logits[logits >= 0.5] = 1
        logits[logits < 0.5] = 0
                ...
                for subject_ in subjects:
                    for object_ in objects:
                        ...
                        if id_ in [8, 10, 32, 46] and id_ + 1 in subject_id_list:
                            id_affi = id_ + 1
                            object_dict[
                                id2spo["object_type"][id_affi].split("_")[1]
                            ] = find_entity(
                                text_raw,
                                id_affi + 55,
                                predictions,
...
def get_precision_recall_f1(golden_file, predict_file):
    r = os.popen(
        "python3 ./re_official_evaluation.py --golden_file={} --predict_file={}".format(
            golden_file, predict_file
        )
    )
    result = r.read()
    r.close()
    precision = float(
        re.search('"precision", "value":.*?}', result)
        .group(0)
        .lstrip('"precision", "value":')
        .rstrip("}")
    )
    recall = float(
        re.search('"recall", "value":.*?}', result)
        .group(0)
        .lstrip('"recall", "value":')
        .rstrip("}")
    )
    f1 = float(
        re.search('"f1-score", "value":.*?}', result)
        .group(0)
        .lstrip('"f1-score", "value":')
        .rstrip("}")
    )
