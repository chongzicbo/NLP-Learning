...
def main_seprate_event(schema_file, schema_name, instances, output_folder):
    ...
    for event in event_map:
        ...
        with open(
            f"{subevent_output_folder}/test.json", "w", encoding="utf8"
        ) as output:
...
def split_test(options):
    ...
    main_entity_relation(
        os.path.join(schema_folder, "人生信息.yaml"),
        "人生信息",
        instances,
        os.path.join(output_folder, "人生信息"),
    )
    main_entity_relation(
        os.path.join(schema_folder, "机构信息.yaml"),
        "机构信息",
        instances,
        os.path.join(output_folder, "机构信息"),
    )
    main_entity_relation(
        os.path.join(schema_folder, "影视情感.yaml"),
        "影视情感",
        instances,
        os.path.join(output_folder, "影视情感"),
    )
    main_event(
        os.path.join(schema_folder, "灾害意外.yaml"),
        "灾害意外",
        instances,
        os.path.join(output_folder, "灾害意外"),
    )
    main_event(
        os.path.join(schema_folder, "体育竞赛.yaml"),
        "体育竞赛",
        instances,
        os.path.join(output_folder, "体育竞赛"),
    )
    main_seprate_event(
        os.path.join(schema_folder, "金融信息.yaml"),
        "金融信息",
        instances,
...
def annonote_graph(
    entities: List[Dict] = [], relations: List[Dict] = [], events: List[Dict] = []
):
    ...
    for relation in relations:
        ...
        add_asoc(
            spot=relation["args"][0], asoc=relation["type"], tail=relation["args"][1]
        )
...
def convert_duuie_to_spotasoc(data_folder, ignore_datasets):
    ...
    for task_folder in os.listdir(data_folder):
        ...
        add_spot_asoc_to_single_file(
            os.path.join(data_folder, task_folder, "train.json.json")
        )
        ...
        for line in open(
            os.path.join(data_folder, task_folder, "train.json.json"), encoding="utf8"
        ):
        ...
        for line in open(
            os.path.join(data_folder, task_folder, "val.json"), encoding="utf8"
        ):
            new_instance = json.loads(line)
            # 添加任务中所有的 Spot 类别
            new_instance["spot"] = record_schema.type_list
...
def dump_event_schema(event_map, output_folder):
    ...
    type_role_map = {
        event_type: list(event_map[event_type]["参数"].keys()) for event_type in event_map
    }
    dump_schema(
        output_folder=output_folder,
        schema_dict={
...
def filter_event_in_instance(instances, required_event_types):
    ...
    for instance in instances:
        ...
        new_instance["event"] = list(
            filter(lambda x: x["type"] in required_event_types, new_instance["event"])
        )
...
def preprocess_duie(data_folder):
    ...
    life_train_instances = load_jsonlines_file(f"{life_folder}/train.json.json")
    org_train_instances = load_jsonlines_file(f"{org_folder}/train.json.json")
    life_relation = RecordSchema.read_from_file(
        f"{life_folder}/record.schema"
    ).role_list
    ...
    with open(f"{life_folder}/train.json.json", "w") as output:
    ...
    with open(f"{org_folder}/train.json.json", "w") as output:
        for instance in instance_dict.values():
            new_instance = copy.deepcopy(instance)
            new_instance["relation"] = list(
...
def preprocess(options):
    ...
    convert_duuie_to_spotasoc(
        data_folder=options.output_folder, ignore_datasets=options.ignore_datasets
    )
...
if __name__ == "__main__":
    ...
    parser_a.add_argument(
        "--data_file", default="data/duuie_test_a.json", help="Path for DuUIE data file"
    )
    parser_a.add_argument(
        "--output_folder",
    ...
    parser_b.add_argument(
        "--submit", default="submit.txt", help="Path for output submission file"
    )
