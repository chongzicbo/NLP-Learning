        ...
        for pred_record in instance:
            ...
            for matched in matched_list:
                if (record_type, matched) not in entity_matched_set:
                    entity_list += [
                        (record_type, tuple(range(matched[0], matched[1] + 1)))
                    ]
...
class RelationRecord(Record):
    ...
    def record_to_offset_first_role(self, instance, token_list):
        ...
        for record in instance:
            ...
            if len(relation) != 5 or (
                self._map_config.de_duplicate and tuple(relation) in relation_list
            ):
                continue
            relation_list += [tuple(relation)]
        ...
        for record in instance:
            ...
            for role_type, text_str in record["roles"]:
                ...
                if len(matched_list) == 0:
                    ...
                    abs_distances = [
                        abs(match[0] - trigger_offset[0]) for match in matched_list
                    ]
                    closest_index = numpy.argmin(abs_distances)
                    pred_record["roles"] += [
                        (role_type, get_index_tuple(matched_list[closest_index]))
                    ]
...
class SEL2Record:
    ...
    def sel2record(self, pred, text, tokens):
        ...
        pred_records = proprocessing_graph_record(
            well_formed_list[0], self._schema_dict
        )
...
def fix_unk_from_text_with_tokenizer(span, text, tokenizer, unk="<unk>"):
    ...
    tokenized_span = tokenizer.encode(
        span, add_special_tokens=False, return_token_type_ids=None
    )["input_ids"]
    tokenized_text = tokenizer.encode(
        text, add_special_tokens=False, return_token_type_ids=None
    )["input_ids"]
...
class SpotAsocPredictParser:
    ...
    def decode(
        ...
        for gold, pred, text, raw_data in zip(
...
def evaluate_extraction_results(
    gold_instances, pred_instances, eval_match_mode="normal"
):
    task_scorer_dict = {
        "entity": EntityScorer,
