...
class Scorer:
    ...
    def eval_instance_list(
        gold_instance_list, pred_instance_list, verbose=False, match_mode="normal"
    ):
        raise NotImplementedError
        ...
        for pred, gold in zip(pred_instance_list, gold_instance_list):
            ...
            for eval_key in metrics:
                metrics[eval_key].count_instance(
                    gold_list=gold.get(eval_key, []), pred_list=pred.get(eval_key, [])
                )

            post_string_tp, post_offset_tp = metrics["string"].tp, metrics["offset"].tp
            if (
                verbose
                and post_offset_tp - pre_offset_tp != post_string_tp - pre_string_tp
            ):
...
class RelationScorer(Scorer):
    ...
    def eval_instance_list(
        ...
        for pred, gold in zip(pred_instance_list, gold_instance_list):
            ...
            post_string_tp, post_offset_tp = metrics["string"].tp, metrics["offset"].tp
            if verbose and (
                post_offset_tp - pre_offset_tp != post_string_tp - pre_string_tp
            ):
                warning_tp_increment(gold=gold, pred=pred, prefix="Relation Strict")

            pre_string_tp, pre_offset_tp = (
                boundary_metrics["string"].tp,
        ...
        for eval_key in metrics:
            results.update(
                metrics[eval_key].compute_f1(prefix=eval_key + "-rel-strict-")
            )
        for eval_key in boundary_metrics:
            results.update(
                boundary_metrics[eval_key].compute_f1(
...
class EventScorer(Scorer):
            ...
            for record in gold:
                ...
                for arg in record["args"]:
                    gold_instance["offset_role"] += [
                        (record["type"], arg["type"], tuple_offset(arg["offset"]))
                    ]
        ...
        for pred in pred_list:
            ...
            for string_pred in pred["string"]:
                ...
                for role_type, role_string in string_pred["roles"]:
                    pred_instance["string_role"] += [
                        (event_type, role_type, role_string)
                    ]
    ...
    def eval_instance_list(
        ...
        for pred, gold in zip(pred_instance_list, gold_instance_list):
            ...
            pre_string_tp, pre_offset_tp = (
                role_metrics["string"].tp,
                role_metrics["offset"].tp,
            )
        ...
        for eval_key in trigger_metrics:
            results.update(
                trigger_metrics[eval_key].compute_f1(prefix=f"{eval_key}-evt-trigger-")
            )
        for eval_key in role_metrics:
            results.update(
                role_metrics[eval_key].compute_f1(prefix=f"{eval_key}-evt-role-")
            )
