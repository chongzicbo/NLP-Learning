...
def parse_args():
    ...
    parser.add_argument(
        "--max_prefix_length", default=None, type=int, help="The maximum prefix length."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--adam_beta1", default=0.9, type=float, help="Beta1 for AdamW optimizer"
    )
    parser.add_argument(
        "--adam_beta2", default=0.999, type=float, help="Beta2 for AdamW optimizer"
    )
    parser.add_argument(
...
def evaluate(
    ...
    for batch in data_loader:
        ...
        outputs = tokenizer.batch_decode(
            outputs, clean_up_tokenization_spaces=False, skip_special_tokens=False
        )
    ...
    for predicted_sel, instance in zip(all_preds, eval_instances):
        record = sel2record.sel2record(
            pred=predicted_sel, text=instance["text"], tokens=instance["tokens"]
        )
        all_records += [record]
...
def train(args, model, tokenizer):
    ...
    eval_tasks = (
        load_eval_tasks(model=model, tokenizer=tokenizer, args=args)
        if args.do_eval
        else None
    )
    ...
    def logging_lr_loss():
        ...
        logger.info(
            f"global_steps {global_steps}/{args.max_steps}"
    ...
    for epoch in range(args.num_train_epochs):
        ...
        save_checkpoint(
            tokenizer, model, os.path.join(args.output_dir, f"ckpt_epoch{epoch}")
        )
        if args.do_eval and paddle.distributed.get_rank() == 0:
            ...
            if current_score > best_score:
                ...
                save_checkpoint(
                    tokenizer, model, os.path.join(args.output_dir, f"best")
                )
