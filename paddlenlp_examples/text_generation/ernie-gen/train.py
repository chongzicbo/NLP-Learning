...
parser.add_argument(
    "--max_encode_len", type=int, default=5, help="The max encoding sentence length"
)
parser.add_argument(
    "--max_decode_len", type=int, default=5, help="The max decoding sentence length"
)
parser.add_argument(
    "--batch_size",
    default=8,
...
parser.add_argument(
    "--warmup_proportion", default=0.1, type=float, help="Linear warmup proportion."
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    choices=["cpu", "gpu", "xpu"],
    help="The device to select to train.json the model, is must be cpu/gpu/xpu.",
)
parser.add_argument("--beam_width", type=int, default=1, help="Beam search width.")
parser.add_argument(
    "--noise_prob", type=float, default=0.0, help="Probability of token be repalced."
)
parser.add_argument(
    "--use_random_noice",
    action="store_true",
...
parser.add_argument(
    "--label_smooth", type=float, default=0.0, help="The soft label smooth rate."
)
parser.add_argument(
    "--length_penalty",
    type=float,
...
parser.add_argument(
    "--init_checkpoint", type=str, default=None, help="Checkpoint to warm start from."
)
parser.add_argument(
...
def evaluate(model, data_loader, tokenizer, rouge1, rouge2, attn_id, tgt_type_id, args):
    ...
    for data in tqdm(data_loader):
        (
            ...
            _,
            raw_tgt_labels,
        ) = data  # never use target when infer
...
def train():
    ...
    train_dataset, dev_dataset = load_dataset(
        "poetry", splits=("train.json", "dev"), lazy=False
    )
    attn_id = (
        tokenizer.vocab["[ATTN]"]
        if "[ATTN]" in tokenizer.vocab
        else tokenizer.vocab["[MASK]"]
    )
    ...
    for epoch in range(args.num_epochs):
        for step, batch in enumerate(train_data_loader):
            ...
            if (
                ...
                model_to_save = (
                    model._layers if isinstance(model, paddle.DataParallel) else model
                )
