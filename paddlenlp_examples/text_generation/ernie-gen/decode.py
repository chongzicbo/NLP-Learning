...
BeamSearchOutput = namedtuple(
    "BeamSearchOutput", ["scores", "predicted_ids", "beam_parent_ids"]
)
...
def mask_prob(p, onehot_eos, finished):
    ...
    p = (
        is_finished * (1.0 - paddle.cast(onehot_eos, "float32")) * -9999.0
        + (1.0 - is_finished) * p
    )
    return p
...
def beam_search_step(state, logits, eos_id, beam_width, is_first_step, length_penalty):
    ...
    onehot_eos = paddle.cast(
        nn.functional.one_hot(paddle.ones([1], "int64") * eos_id, vocab_size), "int64"
    )  # [1, V]
    ...
    if is_first_step:
        allscore = paddle.reshape(allscore, [bsz, beam_width, -1])[
            :, 0, :
...
def beam_search_infilling(
    ...
    def reorder_(t, parent_id):
        ...
        gather_idx = paddle.nonzero(parent_id != -1)[
            :, 0
        ] * beam_width + paddle.reshape(parent_id, [-1])
    ...
    for step in range(max_decode_len):
        ...
        pos_ids = paddle.to_tensor(
            np.tile(
                np.array([[step, step + 1]], dtype=np.int64), [d_batch * beam_width, 1]
            )
        )
        pos_ids += seqlen
        _, logits, info = model(
    ...
    final_ids = nn.functional.gather_tree(final_ids, final_parent_ids)[
        :, :, 0
    ]  # pick best beam
