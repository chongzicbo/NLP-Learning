...
class Demo:
    def __init__(
        self, model_name_or_path="ctrl", max_predict_len=128, repetition_penalty=1.2
    ):
    ...
    def generate(self, inputs, max_predict_len=None, repetition_penalty=None):
        max_predict_len = (
            max_predict_len if max_predict_len is not None else self.max_predict_len
        )
        repetition_penalty = (
            repetition_penalty
        ...
        outputs = self.model.generate(
            input_ids, max_length=max_length, repetition_penalty=self.repetition_penalty
        )[0][0]
