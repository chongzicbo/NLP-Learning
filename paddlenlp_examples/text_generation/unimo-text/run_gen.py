...
def parse_args():
    ...
    parser.add_argument("--do_train", action='store_true', help="Whether to train.json the model.")
...
def run(args):
    ...
    train_ds = load_dataset(
        args.dataset_name, splits="train.json", data_files=args.train_file
    )
    ...
    train_ds, train_data_loader = create_data_loader(
        train_ds, tokenizer, args, "train.json"
    )
...
def evaluation(model, data_loader, args, tokenizer):
    ...
    for step, inputs in enumerate(data_loader, 1):
        ...
        results = select_sum(
            ids, scores, tokenizer, args.max_dec_len, args.num_return_sequences
        )
