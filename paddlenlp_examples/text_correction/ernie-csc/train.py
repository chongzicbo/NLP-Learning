...
from util import convert_example, create_dataloader, read_train_ds
...
parser.add_argument("--learning_rate", type=float, default=5e-5, help="Learning rate used to train.json.")
...
parser.add_argument("--extra_train_ds_dir", default=None, type=str, help="The directory of extra train.json dataset.")
...
def evaluate(model, eval_data_loader):
    ...
    for step, batch in enumerate(eval_data_loader, start=1):
        ...
        corr_metric.update(
            det_error_probs, det_labels, corr_logits, corr_labels, length
        )
...
def do_train(args):
    ...
    train_ds, eval_ds = load_dataset("sighan-cn", splits=["train.json", "dev"])
    ...
    if args.extra_train_ds_dir is not None and os.path.exists(args.extra_train_ds_dir):
        ...
        for data_file in data_files:
            ds = load_dataset(
                read_train_ds, data_path=data_file, splits=["train.json"], lazy=False
            )
            data += ds.data
    ...
    batchify_fn = lambda samples, fn=Tuple(
        ...
        Pad(
            axis=0, pad_val=pinyin_vocab.token_to_idx[pinyin_vocab.pad_token]
        ),  # pinyin
    ...
    num_training_steps = (
        args.max_steps if args.max_steps > 0 else len(train_data_loader) * args.epochs
    )
    ...
    for epoch in range(args.epochs):
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            corr_loss = corr_loss_act(corr_logits, corr_labels) * det_error_probs.max(
                axis=-1
            )
            ...
            if global_steps % args.save_steps == 0:
                if paddle.distributed.get_rank() == 0:
                    ...
                    if f1 > best_f1:
                        ...
                        paddle.save(
                            model.state_dict(),
                            os.path.join(args.output_dir, "best_model.pdparams"),
                        )
                    ...
                    paddle.save(
                        model.state_dict(), os.path.join(args.output_dir, model_file)
                    )
