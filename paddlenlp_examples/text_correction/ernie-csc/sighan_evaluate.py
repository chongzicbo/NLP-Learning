...
def main(args):
    ...
    for idx, (pred, truth) in enumerate(zip(fpred, ftruth)):
        ...
        detect_truth_positions = [
            ...
            for i, truth_token in enumerate(truth_tokens)
            if i % 2 == 0
        ]
    ...
    print("\nDetection Level")
    print(
        "\tPrecision = {:.4f} ({}/{})".format(detect_precision, detect_tp, detect_pos)
    )
    ...
    print(
        "\tPrecision = {:.4f} ({}/{})".format(
            correct_precision, correct_tp, correct_pos
        )
    )
