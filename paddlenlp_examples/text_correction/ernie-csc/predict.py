...
class Predictor(object):
    def __init__(
        self, model_file, params_file, device, max_seq_length, tokenizer, pinyin_vocab
    ):
    ...
    def predict(self, data, batch_size=1):
        ...
        batchify_fn = lambda samples, fn=Tuple(
            ...
            Pad(
                axis=0, pad_val=self.tokenizer.pad_token_type_id, dtype="int64"
            ),  # segment
            Pad(
                axis=0,
                pad_val=self.pinyin_vocab.token_to_idx[self.pinyin_vocab.pad_token],
                dtype="int64",
            ),  # pinyin
        ...
        batch_texts = [
            texts[idx : idx + batch_size] for idx in range(0, len(examples), batch_size)
        ]
        results = []
