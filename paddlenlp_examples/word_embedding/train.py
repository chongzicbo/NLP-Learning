...
parser.add_argument("--device", type=str, default="gpu", help="Select cpu, gpu, xpu devices to train.json model.")
parser.add_argument("--lr", type=float, default=5e-4, help="Learning rate used to train.json.")
...
parser.add_argument("--embedding_name", type=str, default="w2v.baidu_encyclopedia.target.word-word.dim300",
...
def create_dataloader(
    dataset, trans_fn=None, mode="train.json", batch_size=1, pad_token_id=0
):
    """
    Creats dataloader.
    Args:
        dataset(obj:`paddle.io.Dataset`): Dataset instance.
        mode(obj:`str`, optional, defaults to obj:`train.json`): If mode is 'train.json', it will shuffle the dataset randomly.
...
if __name__ == "__main__":
    ...
    if "[PAD]" not in vocab:
        vocab["[PAD]"] = len(vocab)
    # Loads dataset.
    train_ds, dev_ds = load_dataset("chnsenticorp", splits=["train.json", "dev"])
    ...
    trans_fn = partial(
        data.convert_example, vocab=vocab, unk_token_id=vocab["[UNK]"], is_test=False
    )
