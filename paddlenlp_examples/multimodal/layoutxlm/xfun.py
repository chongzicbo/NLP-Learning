...
class XFUN(Dataset):
    ...
    def pad_sentences(
        ...
        if needs_to_be_padded:
            ...
            if self.tokenizer.padding_side == "right":
                ...
                encoded_inputs["input_ids"] = (
                    encoded_inputs["input_ids"]
                    + [self.tokenizer.pad_token_id] * difference
                )
                encoded_inputs["labels"] = (
                    encoded_inputs["labels"] + [self.pad_token_label_id] * difference
                )
                encoded_inputs["bbox"] = (
                    encoded_inputs["bbox"] + [[0, 0, 0, 0]] * difference
                )
                ...
                encoded_inputs["bbox"] = [[0, 0, 0, 0]] * difference + encoded_inputs[
                    "bbox"
                ]
        else:
            if return_attention_mask:
    ...
    def _read_encoded_inputs_sample(self, info_str):
        ...
        if self.contains_re:
            relations = self._relations(
                entities, relations, id2label, empty_entity, entity_id_to_index_map
            )
    ...
    def _relations(
        self, entities, relations, id2label, empty_entity, entity_id_to_index_map
    ):
        """
        build relations
        """
