...
def train(args):
    ...
    train_dataset = FunsdDataset(
        args, tokenizer, all_labels, pad_token_label_id, mode="train.json"
    )
    ...
    logger.info(
        "  Total train.json batch size (w. parallel, distributed & accumulation) = %d",
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = loss.mean()
            logger.info("train.json loss: {}".format(loss.numpy()))
            loss.backward()
...
def evaluate(
    args, model, tokenizer, all_labels, loss_fct, pad_token_label_id, mode, prefix=""
):
