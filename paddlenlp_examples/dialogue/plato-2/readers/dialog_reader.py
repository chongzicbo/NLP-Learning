...
class DialogReader(object):
    ...
    def add_cmdline_args(cls, parser):
        ...
        group.add_argument(
            "--file_format", type=str, default="file", choices=["file", "filelist"]
        )
        group.add_argument(
            "--data_format",
            type=str,
    ...
    def _convert_example_to_record(self, example, is_infer):
        ...
        while idx >= 0:
            ...
            if total_token_num > self.max_src_len:
                if self.truncate_first_turn and idx == 0:
                    truncated_ids = s_token_ids_list[idx][
                        : self.max_src_len - total_token_num
                    ]
        ...
        assert (
            len(token_ids) == len(type_ids) == len(pos_ids)
        ...
        def __wrapper__():
            ...
            if reader is None:
                if self.file_format == "filelist":
                    reader = self._read_files(
                        input_file, phase, is_infer, not phase.endswith("test")
                    )
                else:
                    if phase == "train.json":
                        self.total_file = 1
    ...
    def _gen_self_attn_mask(
        ...
        if is_unidirectional:
            for index, mask_data in enumerate(input_mask_data):
                ...
                mask_data[
                    start + shift_len : end + shift_len,
                    start + shift_len : end + shift_len,
                ] = b
        else:
            for index, token_ids in enumerate(batch_token_ids):
                input_mask_data[
                    index, : len(token_ids) + shift_len, : len(token_ids) + shift_len
                ] = 1.0
    ...
    def _pad_batch_records(self, batch_records, is_infer):
        ...
        if is_infer:
            ...
            batch["init_score"] = (
                np.zeros_like(tgt_ids, dtype="float32").reshape(-1, 1).tolist()
            )
            batch["tgt_ids"] = tgt_ids.tolist()
            batch["tgt_pos"] = tgt_pos.tolist()

            batch["tgt_generation_mask"] = batch["generation_mask"][:, 0:1, :].astype(
                "float32"
            )
        else:
