...
class Plato2Encoder(nn.Layer):
    ...
    def forward(
        self, caches, token_ids, type_ids, pos_ids, generation_mask, aux_emb=None
    ):
        out, self_attn_mask = self.gen_input(
...
class NSP(nn.Layer):
    ...
    def forward(self, inputs):
        ...
        out, self_attn_mask = self.gen_input(
            token_ids, type_ids, pos_ids, attention_mask
        )
    ...
    def decode(self, inputs, caches):
        ...
        while step < self.max_dec_len:
            ...
            tgt_generation_mask = paddle.concat(
                [tgt_generation_mask, paddle.unsqueeze(append_mask, 1)], axis=-1
            )
            ...
            # [-1, 1, hidden_size]
            out, caches = self.plato2_encoder(
                caches, tgt_ids, tgt_sent, tgt_pos, tgt_generation_mask
            )
    ...
    def gen_nsp_input(self, token_ids, pred_ids):
        ...
        def __reader__():
            ...
            for i, (raw, pred) in enumerate(zip(token_ids, pred_ids)):
                ...
                example = Example(
                    src=context_tokenized_input, tgt=response_tokenized_input, data_id=i
                )
