...
def preprocess_examples(examples, mode="train.json"):
    """
    ...
    if mode != "test":
        ...
        masked_positions = np.concatenate(
            [
                ...
                + (max_len - example["seq_len"])
                + i * max_len
                for i, example in enumerate(batch_examples)
            ]
        )
        labels = np.concatenate(
            [np.array(example["labels"], dtype="int64") for example in batch_examples]
        )
        return (
            input_ids,
            token_type_ids,
        ...
        seq_len = np.asarray([example["seq_len"] for example in batch_examples]).astype(
            "int32"
        )
...
def create_data_loader(dataset, tokenizer, args, mode):
    ...
    data_loader = DataLoader(
        dataset, batch_sampler=batch_sampler, collate_fn=collate_fn, return_list=True
    )
...
def select_response(
    ids, scores, tokenizer, max_dec_len=None, num_return_sequences=1, keep_space=True
):
    results = []
    group = []
    tmp = []
    if scores is not None:
        ...
        for pred in ids:
            ...
            last_pos = 0
            if (
                max_dec_len is not None and num_token >= max_dec_len
            ) or in_turn_repetition:
                tmp.append([response])
            else:
