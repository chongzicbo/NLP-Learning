...
def print_logs(args, step, logits, labels, loss, total_time, metric):
    if args.task_name in ["udc", "atis_intent", "mrda", "swda"]:
        ...
        print(
            "step %d - loss: %.4f - acc: %.4f - %.3fs/step"
...
def train(args, model, train_data_loader, dev_data_loader, metric, n_procs, rank):
    ...
    print("\nNum train.json examples: %d" % num_examples)
    print("Max train.json steps: %d" % max_train_steps)
    print("Warmup proportion: %.2f" % args.warmup_proportion)

    lr_scheduler = LinearDecayWithWarmup(
        args.learning_rate, max_train_steps, args.warmup_proportion
    )
...
def create_data_loader(args, dataset_class, trans_func, batchify_fn, mode):
    ...
    data_loader = DataLoader(
        dataset, batch_sampler=batch_sampler, collate_fn=batchify_fn, return_list=True
    )
    return data_loader
...
def main(args):
    ...
    if args.do_train:
        train_data_loader = create_data_loader(
            args, dataset_class, trans_func, batchify_fn, "train.json"
        )
        if args.do_eval:
