...
class UDCv1(Dataset):
    ...
    def __init__(self, data_dir, mode="train.json"):
    ...
    def convert_example(cls, example, tokenizer, max_seq_length=512):
        ...
        def _truncate_and_concat(
            text_a: List[str], text_b: str, tokenizer, max_seq_length
        ):
            ...
            if len(tokens_a) > max_seq_length - len(tokens_b) - 3:
                tokens_a = tokens_a[
                    len(tokens_a) - max_seq_length + len(tokens_b) + 3 :
                ]
            tokens, segment_ids = [], []
        ...
        input_ids, segment_ids = _truncate_and_concat(
            text_a, text_b, tokenizer, max_seq_length
        )
...
class DSTC2(Dataset):
    ...
    def read_data(self):
        ...
        if self._mode == "train.json":
            data_path = os.path.join(self._data_dir, "train.json.txt")
        elif self._mode == "dev":
...
def read_da_data(data_dir, mode):
    def _concat_dialogues(examples):
        ...
        for i in range(len(examples)):
            ...
            pre_txt = [
                "%s : %s" % (item[1], item[2]) for item in examples[max(0, i - 5) : i]
            ]
            suf_txt = [
                "%s : %s" % (item[1], item[2])
    ...
    if mode == "train.json":
        data_path = os.path.join(data_dir, "train.json.txt")
    elif mode == "dev":
