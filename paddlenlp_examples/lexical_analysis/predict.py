...
parser.add_argument("--device", default="gpu", type=str, choices=["cpu", "gpu"] ,help="The device to select to train.json the model, is must be cpu/gpu.")
...
def infer(args):
    ...
    for batch in infer_loader:
        ...
        result = parse_result(
            token_ids.numpy(), preds.numpy(), length.numpy(), word_vocab, label_vocab
        )
        results += result
