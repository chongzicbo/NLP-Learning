...
def evaluate(model, metric, data_loader):
    ...
    logger.info(
        "eval precision: %f, recall: %f, f1: %f" % (precision, recall, f1_score)
    )
    model.train()
...
def train(args):
    ...
    train_ds, test_ds = load_dataset(
        datafiles=(
            os.path.join(args.data_dir, "train.json.tsv"),
    ...
    if args.init_checkpoint:
        if os.path.exists(args.init_checkpoint):
            ...
            logger.info(
                "Cannot init checkpoint from %s which doesn't exist"
            ...
            if global_step % args.save_steps == 0 or global_step == last_step:
                if rank == 0:
                    paddle.save(
                        model.state_dict(),
                        os.path.join(
                            args.model_save_dir, "model_%d.pdparams" % global_step
                        ),
                    )
                    ...
                    if args.do_eval:
                        ...
                        if f1_score > max_f1_score:
                            ...
                            paddle.save(
                                model.state_dict(),
                                os.path.join(
                                    args.model_save_dir, "best_model.pdparams"
                                ),
                            )
