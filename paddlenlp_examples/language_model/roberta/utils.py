...
class DataCollatorMLM:
    def __init__(self, tokenizer, batch_pad=None):
        ...
        if batch_pad is None:
            self.batch_pad = lambda samples, fn=Dict(
                {
                    "input_ids": Pad(
                        axis=0, pad_val=self.pad_token_id, dtype="int64"
                    ),  # input
                    # 'token_type_ids': Pad(axis=0, pad_val=0, dtype='int64'),  # segment
                    "special_tokens_mask": Pad(
                        axis=0, pad_val=True, dtype="int64"
                    ),  # segment
                }
            ): fn(samples)
        else:
    ...
    def __call__(self, examples):
        ...
        examples[0], labels = self._mask_tokens(
            examples[0],
            paddle.cast(examples[1], dtype=bool),
            self.mask_token_id,
            self.token_len,
        )
    ...
    def _mask_tokens(
        ...
        indices_replaced = (
            paddle.cast(paddle.bernoulli(paddle.full(labels.shape, 0.8)), dtype=bool)
            & masked_indices
        )
