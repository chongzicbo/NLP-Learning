...
class RoFormerMeanPoolingForSequenceClassification(RoFormerPretrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None):
        last_hidden_state = self.roformer(
            input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask
        )[0]

        mask = (
            (input_ids != self.roformer.pad_token_id)
            .astype(self.classifier.weight.dtype)
            .unsqueeze(-1)
        )
        mean_pooling = paddle.sum(last_hidden_state * mask, axis=1) / paddle.sum(
            mask, axis=1
        )
...
MODEL_CLASSES = {
    ...
    "roformer_mean_pooling": (
        RoFormerMeanPoolingForSequenceClassification,
        RoFormerTokenizer,
    ),
}
...
def convert_example(example, tokenizer, max_seq_length=512):
    ...
    return [data1["input_ids"], data1["token_type_ids"], 1], [
        data2["input_ids"],
        data2["token_type_ids"],
        0,
    ]
...
def do_train(args):
    ...
    train_ds = load_dataset("cail2019_scm", splits="train.json")
    ...
    dev_batch_sampler = paddle.io.BatchSampler(
        dev_ds, batch_size=args.batch_size * 4, shuffle=False
    )
