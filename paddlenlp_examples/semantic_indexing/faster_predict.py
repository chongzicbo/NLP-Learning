...
def parse_args():
    ...
    parser.add_argument(
        "--text_pair_file", type=str, required=True, help="The full path of input file"
    )
    parser.add_argument(
        "--output_emb_size", default=None, type=int, help="output_embedding_size"
    )
    parser.add_argument(
        "--params_path",
        type=str,
...
class SemanticIndexingPredictor(nn.Layer):
    def __init__(
        self, pretrained_model, output_emb_size, bos_id=0, dropout=0, use_fp16=False
    ):
