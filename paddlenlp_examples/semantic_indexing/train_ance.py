...
parser.add_argument("--ann_data_dir", default='./ann_data', type=str,
                    help="The output directory where the ann generated training data will be saved.")
...
def do_train():
    ...
    model = SemanticIndexANCE(
        pretrained_model, margin=args.margin, output_emb_size=args.output_emb_size
    )
    ...
    while global_step < args.max_training_steps:
        ...
        if latest_ann_data_step == -1:
            ...
            logger.info(
                "Latest ann_data is ready for training: [{}]".format(latest_ann_data)
            )
