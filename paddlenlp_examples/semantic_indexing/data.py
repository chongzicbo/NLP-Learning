...
def convert_example(example, tokenizer, max_seq_length=512, pad_to_max_seq_len=False):
    ...
    for key, text in example.items():
        encoded_inputs = tokenizer(
            text=text, max_seq_len=max_seq_length, pad_to_max_seq_len=pad_to_max_seq_len
        )
...
def get_latest_checkpoint(args):
    ...
    if len(trained_steps) > 0:
        return os.path.join(
            args.save_dir, str(max(trained_steps)), "model_state.pdparams"
        ), max(trained_steps)
...
def get_latest_ann_data(ann_data_dir):
    ...
    if len(ann_data_steps) > 0:
        latest_ann_data_file = os.path.join(
            ann_data_dir, str(max(ann_data_steps)), "new_ann_data"
        )
