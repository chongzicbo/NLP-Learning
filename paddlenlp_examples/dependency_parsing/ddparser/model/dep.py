...
class BiAffineParser(nn.Layer):
    ...
    def __init__(
        ...
        # MLP layer
        self.mlp_arc_h = MLP(
            n_in=self.embed.mlp_input_size, n_out=n_mlp_arc, dropout=mlp_dropout
        )
        self.mlp_arc_d = MLP(
            n_in=self.embed.mlp_input_size, n_out=n_mlp_arc, dropout=mlp_dropout
        )
        self.mlp_rel_h = MLP(
            n_in=self.embed.mlp_input_size, n_out=n_mlp_rel, dropout=mlp_dropout
        )
        self.mlp_rel_d = MLP(
            n_in=self.embed.mlp_input_size, n_out=n_mlp_rel, dropout=mlp_dropout
        )
...
class BiAffine(nn.Layer):
    ...
    def forward(self, x, y):
        ...
        x = paddle.expand(
            paddle.unsqueeze(x, axis=1), shape=(x.shape[0], o, x.shape[1], x.shape[2])
        )
        # Shape y: (batch_size, output_size, num_tokens, input_size + bias_y)
        y = paddle.expand(
            paddle.unsqueeze(y, axis=1), shape=(y.shape[0], o, y.shape[1], y.shape[2])
        )
        ...
        s = paddle.matmul(
            paddle.matmul(x, weight), paddle.transpose(y, perm=[0, 1, 3, 2])
        )
