...
def do_train(args):
    ...
    train_ds, dev_ds = load_dataset(args.task_name, splits=["train.json", "dev"])

    # Build the vocabs based on train.json corpus
    ...
    for epoch in range(1, args.epochs + 1):
        for inputs in train_data_loader():
            if (
                args.encoding_model.startswith("ernie")
                or args.encoding_model == "lstm-pe"
            ):
