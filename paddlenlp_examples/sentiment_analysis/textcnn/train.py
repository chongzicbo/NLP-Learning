...
parser.add_argument("--lr", type=float, default=5e-5, help="Learning rate used to train.json.")
parser.add_argument("--save_dir", type=str, default='checkpoints/', help="Directory to save model checkpoint")
parser.add_argument("--data_path", type=str, default='E:\\opensource_data\\分类\\情感分析\\RobotChat\\',
...
if __name__ == '__main__':
    ...
    train_ds, dev_ds, test_ds = [
        load_dataset(read_custom_data, filename=os.path.join(args.data_path, dataset_name), lazy=False) for dataset_name
        in dataset_names]
    tokenizer = JiebaTokenizer(vocab)
    trans_fn = partial(convert_example, tokenizer=tokenizer)
    batchify_fn = lambda samples, fn=Tuple(Pad(axis=0, pad_val=vocab.token_to_idx.get("[PAD]", 0)),
                                           Stack(dtype="int64")): [data for data in fn(samples)]

    train_loader = create_dataloader(train_ds, batch_size=args.batch_size, mode="train.json", batchify_fn=batchify_fn,
                                     trans_fn=trans_fn)
    dev_loader = create_dataloader(dev_ds, batch_size=args.batch_size, mode="validation", batchify_fn=batchify_fn,
                                   trans_fn=trans_fn)
    test_loader = create_dataloader(test_ds, batch_size=args.batch_size, mode="test", batchify_fn=batchify_fn,
                                    trans_fn=trans_fn)
