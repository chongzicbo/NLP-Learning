...
po_m = model.po_model(len(char2id) + 2, CHAR_SIZE, HIDDEN_SIZE, 49).to(device)
    ...
    for i, _kk1 in enumerate(_k1):
        if _kk1 > 0.5:
            ...
            if _subject:
                ...
                _o1, _o2 = po_m(
                    t.to(device), t_max.to(device), _k1.to(device), _k2.to(device)
                )
...
for i in range(EPOCH_NUM):
    ...
    print(
        "f1: %.4f, precision: %.4f, recall: %.4f, bestf1: %.4f, bestepoch: %d \n "
