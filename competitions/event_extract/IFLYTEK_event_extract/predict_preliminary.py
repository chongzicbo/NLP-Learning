...
def trigger_evaluation_v1(model, dev_info, device, **kwargs):
    ...
    for tmp_pred, tmp_callback in zip(pred_logits, dev_callback_info):
        ...
        pred_triggers = pointer_trigger_decode(
            tmp_pred,
            text,
            distant_triggers,
    ...
    metric_str = (
        f"In start threshold: {start_threshold}; end threshold: {end_threshold}\n"
    )
...
def role_evaluation(model, dev_info, device, **kwargs):
    ...
    for tmp_pred, tmp_callback in zip(pred_logits, dev_callback_info):
        ...
        pred_obj = pointer_decode(
            tmp_pred[:, :2], text, start_threshold, end_threshold, True
        )
        pred_sub = pointer_decode(
            tmp_pred[:, 2:], text, start_threshold, end_threshold, True
        )
        ...
        for idx, _role in enumerate(role_types):
            ...
            if True:
                wrong_pred.append(
                    {"role": _role, "pred": pred_roles[_role], "gt": gt_roles[_role]}
                )
        ...
        if len(wrong_pred):
            instances.append(
                {"text": text, "trigger": trigger, "wrong_pred": wrong_pred}
            )
...
def attribution_evaluation(model, dev_info, device, **kwargs):
    ...
    for tmp_pred_tense, tmp_pred_polarity, tmp_callback in zip(
        tense_logits, polarity_logits, dev_callback_info
    ):
        ...
        if pred_tense == gt_attributions[0]:
            ...
            wrong_pred.append(
                {"attribution": "tense", "pred": pred_tense, "gt": gt_attributions[0]}
            )
...
def evaluate(
    opt, dev_file="dev.json", force_one=True, model_name=None, dev_raw_examples=None
):
    processors = {
    ...
    if opt.task_type == "trigger":
        with open(
            os.path.join(opt.mid_data_dir, f"triggers_dict.json"), encoding="utf-8"
        ) as f:
            triggers_dict = json.load(f)
    elif opt.task_type == "attribution":
        with open(
            os.path.join(opt.mid_data_dir, f"polarity2id.json"), encoding="utf-8"
        ) as f:
            polarity2id = json.load(f)
        with open(
            os.path.join(opt.mid_data_dir, f"tense2id.json"), encoding="utf-8"
        ) as f:
    ...
    dev_loader = DataLoader(
        dev_dataset, batch_size=opt.eval_batch_size, shuffle=False, num_workers=8
    )
    ...
    for idx, model_path in enumerate(model_path_list):
        ...
        if opt.task_type == "trigger":
            ...
            tmp_metric_str, tmp_f1, tmp_instances = trigger_evaluation_v1(
                model,
                dev_info,
                device,
            ...
            tmp_metric_str, tmp_f1, tmp_instances = role_evaluation(
                model,
                dev_info,
                device,
                start_threshold=opt.start_threshold,
            ...
            (
                ...
                model, dev_info, device, polarity2id=polarity2id, tense2id=tense2id
            )
        ...
        if opt.task_type == "attribution":
            pk.dump(
                [polarity_logits, tense_logits],
                open(os.path.join(tmp_model_dir, "logits.json"), "wb"),
            )
        with open(
            os.path.join(tmp_model_dir, "instance.json"), "w", encoding="utf-8"
        ) as f:
...
# In[26]:
trigger_check_file = list(
    sorted(
        [e for e in os.listdir(dev_trigger_dir) if "checkpoint-" in e],
        key=lambda x: int(x.split("-")[1]),
    )
)[-1]
role_check_file = list(
    sorted(
        [e for e in os.listdir(dev_role_dir) if "checkpoint-" in e],
        key=lambda x: int(x.split("-")[1]),
    )
)[-1]
...
trigger_pred_info = load_examples(
    os.path.join(dev_trigger_dir, trigger_check_file, "instance.json")
)
for idx in tqdm(range(len(new_stack))):
    pred_triggers = trigger_pred_info[idx]["pred"]
    new_stack[idx]["pred_triggers"] = [
        {"text": e[0], "length": len(e[0]), "offset": int(e[1])} for e in pred_triggers
    ]
...
role_pred_info = load_examples(
    os.path.join(dev_role_dir, role_check_file, "instance.json")
)
text_idx = 0
