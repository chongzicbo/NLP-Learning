...
from src_final.utils.model_utils import (
    TriggerExtractor,
    Role2Extractor,
    Role1Extractor,
...
def pipeline_predict(opt):
    ...
    role1_model = Role1Extractor(
        bert_dir=opt.bert_dir, use_trigger_distance=opt.role1_use_trigger_distance
    )

    role2_model = Role2Extractor(
        bert_dir=opt.bert_dir, use_trigger_distance=opt.role2_use_trigger_distance
    )

    attribution_model = AttributionClassifier(bert_dir=opt.bert_dir)

    logger.info("Load models")
    trigger_model, device = load_model_and_parallel(
        trigger_model,
        opt.gpu_ids[0],
        ckpt_path=os.path.join(opt.trigger_ckpt_dir, "model.pt"),
    )

    role1_model, _ = load_model_and_parallel(
        role1_model,
        opt.gpu_ids[0],
        ckpt_path=os.path.join(opt.role1_ckpt_dir, "model.pt"),
    )

    role2_model, _ = load_model_and_parallel(
        role2_model,
        opt.gpu_ids[0],
    ...
    with torch.no_grad():
        ...
        for _ex in tqdm(text_examples, desc="decode test examples"):
            ...
            for _trigger in tmp_triggers:
                ...
                # 此处 start end 与新一版本的模型不一致，此版本是正确的
    ...
    with open(
        os.path.join(opt.submit_dir, f"submit_{opt.version}.json"),
        "w",
        encoding="utf-8",
    ) as f:
