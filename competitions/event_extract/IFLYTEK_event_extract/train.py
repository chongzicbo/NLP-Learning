...
def train_base(opt, info_dict, train_examples, dev_info=None):
    ...
    train_features = convert_examples_to_features(
        opt.task_type, train_examples, opt.bert_dir, opt.max_seq_len, **feature_para
    )

    logger.info(f"Build {len(train_features)} train features")

    train_dataset = build_dataset(
        opt.task_type, train_features, "train", **dataset_para
    )
    ...
    if dev_info is not None:
        ...
        dev_features = convert_examples_to_features(
            opt.task_type, dev_examples, opt.bert_dir, opt.max_seq_len, **feature_para
        )
...
def training(opt):
    ...
    if opt.enhance_data and opt.task_type in ["trigger", "role1", "role2"]:
        ...
        if opt.task_type in ["trigger", "role1"]:
            train_aux_raw_examples = processor.read_json(
                os.path.join(opt.aux_data_dir, f"{opt.task_type}_first.json")
            )
        ...
        if opt.task_type == "role1":
            ...
            train_aux_raw_examples = processor.read_json(
                os.path.join(opt.aux_data_dir, f"{opt.task_type}_second.json")
            )
            train_examples += processor.get_train_examples(train_aux_raw_examples)
        # time & loc 用初赛全部数据进行增强
        elif opt.task_type == "role2":
            train_aux_raw_examples = processor.read_json(
                os.path.join(opt.raw_data_dir, "preliminary_stack.json")
            )
            train_examples += processor.get_train_examples(train_aux_raw_examples)
        # trigger 用更正的第三部分数据进行增强
            ...
            train_aux_raw_examples = processor.read_json(
                os.path.join(opt.aux_data_dir, f"{opt.task_type}_third_new.json")
            )
            train_examples += processor.get_train_examples(train_aux_raw_examples)
...
def stacking(opt):
    ...
    stack_raw_examples = processor.read_json(
        os.path.join(opt.raw_data_dir, "stack.json")
    )
...
if __name__ == "__main__":
    ...
    args.output_dir = os.path.join(
        args.output_dir, mode, args.task_type, args.bert_type
    )
    ...
    if args.mode == "stack":
        assert args.task_type in [
            "attribution"
        ], "Only support attribution task to stack"
        stacking(args)
    else:
        training(args)
