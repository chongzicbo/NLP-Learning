...
def ensemble_attribution(version):
    ...
    # 需要进行 ensemble 的最优文件
    with open(
        os.path.join(SUBMIT_DIR, f"submit_{version}.json"), encoding="utf-8"
    ) as f:
    ...
    with torch.no_grad():
        for idx, _model_path in enumerate(ernie_models_path):
            ...
            ernie_model.load_state_dict(
                torch.load(_model_path, map_location=torch.device("cpu"))
            )
            ernie_model.eval()
            ernie_model.to(device)

            ernie_polarity_logits, ernie_tense_logits = base_attribution_predict(
                examples, ernie_model, device, ernie_tokenizer, f"ernie {idx}"
            )
    ...
    with open(
        os.path.join(SUBMIT_DIR, f"submit_{version}_ensemble.json"),
        "w",
        encoding="utf-8",
    ) as f:
