...
def find_pair(before, role, nexts, pair=("《", "》")):
    ...
    if "”" in pair or "(" in pair or "'" in pair or "（" in pair:
        if (
            (not (len(pre) > 1 and len(sub) > 1))
            and (not len(pre) + len(sub) == 0)
            and (len(pre) == 0 or len(sub) == 0)
        ):
            ...
            for kdx, argument in enumerate(event["arguments"]):
                if argument["role"] == "subject" or argument["role"] == "object":
                    for pair in [
                        ...
                        before = text[
                            max(argument["offset"] - 40, 0) : argument["offset"]
                        ]
                        nexts = text[
                            argument["offset"]
                            + argument["length"] : min(
                        ...
                        if pre != "" or sub != "":
                            ...
                            assert (
                                text[new_offset : new_offset + new_length] == new_text
                            )
...
def convert_raw_data(data_dir, save_data=False, save_dict=False):
    ...
    preliminary_examples = load_examples(
        os.path.join(raw_dir, "preliminary_pred_triggers_pred_roles.json")
    )
    ...
    train, dev = train_test_split(
        stack_examples, shuffle=True, random_state=456, test_size=0.2
    )
...
def split_preliminary_trigger_data(base_dir, save_dir):
    ...
    preliminary_examples = load_examples(
        os.path.join(base_dir, "preliminary_stack.json")
    )
    first_examples = []
    second_examples = []
    third_examples = []
    ...
    for _ex in tqdm(preliminary_examples, desc="process preliminary examples"):
        ...
        if len(_ex["events"]) > 1:
            ...
            if (
                gt_trigger["text"] == pred_trigger["text"]
                and gt_trigger["offset"] == pred_trigger["offset"]
            ):
                first_examples.append(_ex)
            else:
...
def split_preliminary_trigger_third_data(bert_dir, base_dir, ckpt_path):
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(ckpt_path, "model.pt"), map_location=torch.device("cpu")
        )
    )
    ...
    with torch.no_grad():
        for _ex in tqdm(trigger_third, desc="process trigger third"):
            ...
            trigger_pred = pointer_trigger_decode(
                trigger_pred_logits,
                text,
                distant_triggers,
                start_threshold=0.5,
            ...
            tmp_ex = {
                "sentence": text,
                "distant_triggers": distant_triggers,
                "events": [],
            }

            for _label in trigger_labels:
                if (
                    _label["text"] == trigger_pred[0][0]
                    and _label["offset"] == trigger_pred[0][1]
                ):
                    tmp_ex["events"].append(
                        {
                            "trigger": {
                                "text": _label["text"],
...
def split_preliminary_role_data(base_dir, save_dir):
    ...
    for _ex in tqdm(
        preliminary_examples, desc="process preliminary examples with roles"
    ):
        tmp_first_event = {"sentence": _ex["sentence"], "events": [], "pred_events": []}
        ...
        for _event in _ex["events"]:
            for _pred in _ex["pred_events"]:
                ...
                if (
                    _pred["trigger"]["text"] == _event["trigger"]["text"]
                    ...
                    for _aug in _event["arguments"]:
                        ...
                        for _pred_aug in _pred["arguments"]:
                            if _pred_aug["role"] == _aug["role"]:
                                if (
                                    _pred_aug["text"] == _aug["text"]
                                    and _pred_aug["offset"] == _aug["offset"]
                                ):
                                    tag1 = True
                                else:
                                    # 存在某一个被包含
...
if __name__ == "__main__":
    ...
    split_preliminary_trigger_data(
        "../../data/final/raw_data", "../../data/final/preliminary_clean"
    )
    split_preliminary_role_data(
        "../../data/final/raw_data", "../../data/final/preliminary_clean"
    )
