...
def convert_raw_data(data_dir, save_data=False, save_dict=False, use_clean=False):
    ...
    if not use_clean:
        ...
        preliminary_examples = load_examples(
            os.path.join(raw_dir, "raw_preliminary.json")
        )
    else:
        stack_examples = load_examples(os.path.join(raw_dir, "raw_stack_clean.json"))
        preliminary_examples = load_examples(
            os.path.join(raw_dir, "raw_preliminary_clean.json")
        )
    ...
    train, dev = train_test_split(
        stack_examples, shuffle=True, random_state=123, test_size=0.15
    )
...
if __name__ == "__main__":
    stack = []
    with open(
        os.path.join("../../data/preliminary/raw_data/", "stack.json"), encoding="utf-8"
    ) as f:
    ...
    for sample in tqdm(stack):
        new_sample = {
            ...
            "events": [],
            "distant_triggers": sample["distant_trigger"],
        }
        for event in sample["labels"]:
            new_event = {
                "trigger": {
                    "text": event["trigger"][0],
                    "length": len(event["trigger"][0]),
                    "offset": event["trigger"][1],
                },
                "tense": "过去",
                "polarity": "肯定",
                "arguments": [],
