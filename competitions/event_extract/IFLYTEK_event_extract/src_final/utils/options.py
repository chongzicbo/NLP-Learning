...
class BaseArgs:
    ...
    def initialize(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--raw_data_dir",
            default="./data/final/raw_data",
            help="the data dir of raw data",
        )

        parser.add_argument(
            "--mid_data_dir", default="./data/final/mid_data", help="the mid data dir"
        )

        parser.add_argument(
            "--aux_data_dir",
            default="./data/final/preliminary_clean",
        ...
        parser.add_argument(
            "--bert_dir",
            default="e:/opensource_data/pretrained_model/huggingface/chinese-bert-wwm-ext",
        ...
        parser.add_argument(
            "--swa_start", default=1, type=int, help="the epoch when swa start"
        )
...
class TrainArgs(BaseArgs):
    ...
    def initialize(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--train_epochs", default=10, type=int, help="Max training epoch"
        )

        parser.add_argument(
            "--dropout_prob", default=0.1, type=float, help="drop out probability"
        )

        parser.add_argument(
            "--lr", default=2e-5, type=float, help="learning rate for the bert module"
        )
        ...
        parser.add_argument(
            "--max_grad_norm", default=1.0, type=float, help="max grad clip"
        )
...
class TestArgs(BaseArgs):
    ...
    def initialize(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--role1_use_trigger_distance", default=False, action="store_true"
        )

        parser.add_argument(
            "--role2_use_trigger_distance", default=False, action="store_true"
        )
