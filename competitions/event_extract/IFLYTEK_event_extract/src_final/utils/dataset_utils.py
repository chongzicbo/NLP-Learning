...
class BaseDataset(Dataset):
    def __init__(self, features, mode):
        ...
        self.token_ids = [
            torch.tensor(example.token_ids).long() for example in features
        ]
        self.attention_masks = [
            torch.tensor(example.attention_masks).float() for example in features
        ]
        self.token_type_ids = [
            torch.tensor(example.token_type_ids).long() for example in features
        ]
...
class TriggerDataset(BaseDataset):
    def __init__(self, features, mode, use_distant_trigger=False):
        super(TriggerDataset, self).__init__(features, mode)

        self.distant_trigger = None

        if use_distant_trigger:
            self.distant_trigger = [
                torch.tensor(example.distant_trigger_label).long()
...
class RoleDataset(BaseDataset):
    def __init__(self, features, mode, use_trigger_distance=False):
        super(RoleDataset, self).__init__(features, mode)
        self.trigger_distance = None

        self.trigger_label = [
            torch.tensor(example.trigger_loc).long() for example in features
        ]
        if use_trigger_distance:
            self.trigger_distance = [
                torch.tensor(example.trigger_distance).long() for example in features
            ]
...
class AttributionDataset(BaseDataset):
    def __init__(self, features, mode):
        ...
        self.pooling_masks = [
            torch.tensor(example.pooling_masks).float() for example in features
        ]
...
def build_dataset(task_type, features, mode, **kwargs):
    ...
    if task_type == "trigger":
        dataset = TriggerDataset(
            features, mode, use_distant_trigger=kwargs.pop("use_distant_trigger")
        )
    elif task_type in ["role1", "role2"]:
        dataset = RoleDataset(
            features, mode, use_trigger_distance=kwargs.pop("use_trigger_distance")
        )
