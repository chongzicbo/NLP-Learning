...
def get_base_out(model, loader, device, task_type):
    ...
    with torch.no_grad():
        for idx, _batch in enumerate(
            tqdm(loader, desc=f"Get {task_type} task predict logits")
        ):
...
def pointer_trigger_decode(
        ...
        for _end in end_ids:
            ...
            if _end >= _start and _end - _start <= 2:
                ...
                candidate_entities.append(
                    (
                        raw_text[_start : _end + 1],
                        _start,
                        logits[_start][0] + logits[_end][1],
                    )
                )
                break
    ...
    if len(candidate_entities):
        candidate_entities = sorted(
            candidate_entities, key=lambda x: x[-1], reverse=True
        )
...
def pointer_decode(
    logits, raw_text, start_threshold=0.5, end_threshold=0.5, force_decode=False
):
    """
    ...
    for _start in start_ids:
        for _end in end_ids:
            if _end >= _start:
                # (start, end, logits)
                candidate_entities.append(
                    (_start, _end, logits[_start][0] + logits[_end][1])
                )
                break
