...
class ConditionalLayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-12):
        ...
        self.weight_dense = nn.Linear(
            normalized_shape * 2, normalized_shape, bias=False
        )
...
class BaseModel(nn.Module):
    ...
    def _batch_gather(data: torch.Tensor, index: torch.Tensor):
        ...
        """
        index = index.unsqueeze(-1).repeat_interleave(
            data.size()[-1], dim=-1
        )  # (bs, n, hidden)
...
class TriggerExtractor(BaseModel):
    def __init__(self, bert_dir, dropout_prob=0.1, use_distant_trigger=False, **kwargs):
        super(TriggerExtractor, self).__init__(
            bert_dir=bert_dir, dropout_prob=dropout_prob
        )
        ...
        if use_distant_trigger:
            ...
            self.distant_trigger_embedding = nn.Embedding(
...
class Role1Extractor(BaseModel):
    def __init__(
        self, bert_dir, dropout_prob=0.1, use_trigger_distance=False, **kwargs
    ):
        super(Role1Extractor, self).__init__(
            bert_dir=bert_dir, dropout_prob=dropout_prob
        )
        ...
        self.conditional_layer_norm = ConditionalLayerNorm(
            out_dims, eps=self.bert_config.layer_norm_eps
        )

        if use_trigger_distance:
            embedding_dim = kwargs.pop("embedding_dims", 256)

            self.trigger_distance_embedding = nn.Embedding(
                num_embeddings=512, embedding_dim=embedding_dim
            )
    ...
    def forward(
        ...
        trigger_label_feature = trigger_label_feature.view(
            [trigger_label_feature.size()[0], -1]
        )
...
class Role2Extractor(BaseModel):
    def __init__(
        ...
        super(Role2Extractor, self).__init__(
            bert_dir=bert_dir, dropout_prob=dropout_prob
        )
    ...
    def forward(
        ...
        if labels is not None:
            tokens_loss = -1.0 * self.crf_module(
                emissions=emissions,
                tags=labels.long(),
...
class AttributionClassifier(BaseModel):
    ...
    def forward(
        ...
        out = (
            ...
            torch.softmax(tense_logits, dim=-1),
        )
...
def build_model(task_type, bert_dir, **kwargs):
    ...
    if task_type == "trigger":
        ...
        model = AttributionClassifier(
            bert_dir=bert_dir, dropout_prob=kwargs.pop("dropout_prob", 0.1)
        )
