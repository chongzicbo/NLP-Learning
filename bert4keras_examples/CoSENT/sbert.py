...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, (text1, text2, label) in self.sample(random):
            label = int(label > 2.5) if random and task_name == "STS-B" else label
...
model.compile(
    loss="sparse_categorical_crossentropy", optimizer=Adam(2e-5), metrics=["accuracy"]
)
...
if __name__ == "__main__":
    ...
    print("test_score: %.5f" % test_score)
