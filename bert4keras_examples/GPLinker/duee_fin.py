...
"""
# 事件抽取任务，基于GPLinker
# DuEE-fin数据集：https://aistudio.baidu.com/aistudio/competition/detail/46/0/datasets
...
# 读取schema
labels, enum_roles, enum_items = [], [], {}
with open('../datasets/duee_fin_event_schema.json') as f:
    for l in f:
        l = json.loads(l)
        t = l['event_type']
        labels.append((t, u'触发词'))
        for s in l['role_list']:
            if 'enum_items' in s:
...
def load_data(filename):
    ...
    with open(filename, encoding='utf-8') as f:
        for l in f:
            l = json.loads(l)
            text = normalize(l['title'] + '\n' + l['text'])
            d = {'text': text, 'events': []}
            for e in l.get('event_list', []):
                d['events'].append([(
                    e['event_type'], u'触发词', normalize(e['trigger']),
...
# 加载数据集
train_data = load_data('../datasets/duee_fin_train.json')
valid_data = load_data('../datasets/duee_fin_dev.json')
    ...
    with open(in_file) as fr:
        for l in tqdm(fr):
            ...
            for event in extract_events(l['text']):
                ...
                for argu in event:
                    if argu[1] != u'触发词':
                        if argu[:2] in enum_items:
                            role = enum_items[argu[:2]][1]
                            argu = {'role': role, 'argument': argu[1]}
                        else:
                            argu = {'role': argu[1], 'argument': argu[2]}
                        final_event['arguments'].append(argu)
                event_list = [
...
if __name__ == '__main__':
    ...
    # predict_to_file('../datasets/duee_fin_test2.json', 'duee-fin.json')
