...
for i, var in enumerate(variants):
    ...
    fs = [
        "../datasets/sohu2021_open_data_clean/%s/train.json.txt" % var,
...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, (cond, source, target, label) in self.sample(random):
            token_ids, segment_ids = tokenizer.encode(source, target, maxlen=maxlen)
            ...
            if len(batch_token_ids) == self.batch_size or is_end:
                ...
                yield [batch_token_ids, batch_segment_ids, batch_conds], batch_labels
                batch_token_ids, batch_segment_ids = [], []
                batch_conds, batch_labels = [], []
...
def evaluate(data):
    ...
    total_a, right_a = 0.0, 0.0
    total_b, right_b = 0.0, 0.0
