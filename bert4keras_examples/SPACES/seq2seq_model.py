...
if os.path.exists(seq2seq_config_json):
    token_dict, keep_tokens, compound_tokens = json.load(open(seq2seq_config_json))
else:
    # 加载并精简词表
    ...
    json.dump(
        [token_dict, keep_tokens, compound_tokens], open(seq2seq_config_json, "w")
    )

tokenizer = Tokenizer(
    token_dict, do_lower_case=True, pre_tokenize=lambda s: jieba.cut(s, HMM=False)
)
...
def random_masking(token_ids):
    ...
    return [
        t if r > 0.15 else np.random.choice(token_ids) for r, t in zip(rands, token_ids)
    ]
...
if __name__ == "__main__":
    ...
    train_data = data_split(data, fold, num_folds, "train.json")
