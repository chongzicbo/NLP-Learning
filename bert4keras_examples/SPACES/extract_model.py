...
def evaluate(data, data_x, threshold=0.2):
    ...
    for d, yp in tqdm(zip(data, y_pred), desc="评估中"):
        yp = yp[: len(d[0])]
...
if __name__ == "__main__":
    ...
    train_x = data_split(data_x, fold, num_folds, "train.json")
    valid_x = data_split(data_x, fold, num_folds, "valid")
    train_y = data_split(data_y, fold, num_folds, "train.json")
    ...
    model.fit(
        train_x, train_y, epochs=epochs, batch_size=batch_size, callbacks=[evaluator]
    )
