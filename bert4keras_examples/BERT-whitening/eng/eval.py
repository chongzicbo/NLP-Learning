...
if task_name == "STS-B":
    ...
    datasets = {
        ...
        "sick-r-test": load_sick_r_data(data_path + "SICK-R/SICK_test_annotated.txt"),
    }
...
if n_components == 0:
    ...
    if normalized_by == "target":
        ...
        if os.path.exists("weights/nli.%s.kernel.bias.npy" % model_type):
            kernel, bias = np.load(
                "weights/nli.%s.kernel.bias.npy" % model_type, allow_pickle=True
            )
        else:
            ...
            nli_a_vecs, nli_b_vecs, _ = convert_to_vecs(nli_data, tokenizer, encoder)
