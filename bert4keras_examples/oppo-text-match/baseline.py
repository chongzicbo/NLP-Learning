...
# 加载数据集
data = load_data(
    os.path.join(data_dir, 'gaiic_track3_round1_train_20210228.tsv')
)
train_data = [d for i, d in enumerate(data) if i % 10 != 0]
valid_data = [d for i, d in enumerate(data) if i % 10 == 0]
test_data = load_data(
    os.path.join(data_dir, 'gaiic_track3_round1_testA_20210228.tsv')
)
...
def sample_convert(text1, text2, label, random=False):
    ...
    output_ids = [label + 5] + out1_ids + [0] + out2_ids + [0]  # label+5：如果label为1，+5=6，6表示yes,5表示no.对于测试数据，其label为-5，+5后=0，0表示[PAD].通过[CLS]向量预测yes或者no
...
model = build_transformer_model(
    ...
    with_mlm=True,
    keep_tokens=[0, 100, 101, 102, 103, 100, 100] + keep_tokens[:len(tokens)]  # keep_tokens为要保留的词id列表
)
