        ...
        for qa in d["qas"]:
            ...
            for a in qa["answers"]:
                if a["answer_start"] == -1:
                    ...
                    D.append(truncate_strings(q, c[:start], c[start:end], c[end:]))
    return D


# 读取数据
train_data = load_data("../datasets/train.json.json")
...
def predict_to_file(in_file, out_file):
    ...
    for d in tqdm(data):
        ...
        for qa in d["qas"]:
            ...
            answers[qa["id"]] = a
    json.dump(
        answers, open(out_file, "w", encoding="utf-8"), indent=4, ensure_ascii=False
    )
...
def evaluate(filename):
    ...
    metrics = json.loads(
        os.popen(
            "python ../datasets/evaluate.py %s %s" % (filename, filename + ".pred.json")
        )
        .read()
        .strip()
    )
