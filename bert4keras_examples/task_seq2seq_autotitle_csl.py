...
def load_data(filename):
    ...
    with open(filename, encoding="utf-8") as f:
        for l in f:
            json_str = json.loads(l.strip())
            try:
                title, content = json_str.get("title", ""), json_str["abst"]
            except KeyError:
                print(json_str)
                continue
...
# 加载数据集
data_dir = "E:\\opensource_data\\文本摘要\\csl_title_public\\csl_title_public\\"
train_data = load_data(os.path.join(data_dir, "csl_title_train.json"))
valid_data = load_data(os.path.join(data_dir, 'csl_title_dev.json'))
test_data = load_data(os.path.join(data_dir, 'csl_title_test.json'))
...
if __name__ == '__main__':
    ...
    evaluator = Evaluator()
    train_generator = data_generator(valid_data, batch_size)
