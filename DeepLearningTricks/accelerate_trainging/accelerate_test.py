...
def create_dataloaders(batch_size=64):
    ...
    dl_val = torch.utils.data.DataLoader(
        ds_val, batch_size=batch_size, shuffle=False, num_workers=2, drop_last=True
    )
...
def training_loop(
    epochs=5, lr=1e-3, batch_size=1024, ckpt_path="checkpoint.pt", mixed_precision="no"
):
    ...
    for epoch in range(epochs):
        ...
        accelerator.print(
            f"epoch【{epoch}】@{nowtime} --> eval_metric= {100 * eval_metric:.2f}%"
        )
