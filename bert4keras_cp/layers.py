...
from bert4keras_cp.backend import keras, K, is_tf_keras
from bert4keras_cp.backend import align, sequence_masking
from bert4keras_cp.backend import align, sequence_masking
from bert4keras_cp.backend import recompute_grad
from bert4keras_cp.backend import attention_normalize
from bert4keras_cp.backend import apply_rotary_position_embeddings
...
# 直接覆盖对象
keras.layers.GlobalAveragePooling1D = GlobalMaxPooling1D
...
class ScaleOffset(Layer):
    def __init__(self, scale=True, offset=True, conditional=False,
                 hidden_units=None, hidden_activation="linear", hidden_initializer='glorot_uniform', **kwargs):
    ...
    def build(self, input_shape):
        ...
        if self.conditional:
            if self.hidden_units is not None:
                self.hidden_dense = Dense(units=self.hidden_units, activation=self.hidden_activation, use_bias=False,
                                          kernel_initializer=self.hidden_initializer)

            if self.offset is not False and self.offset is not None:
                self.beta_dense = Dense(units=input_shape[-1], use_bias=False, kernel_initializer='zeros')
