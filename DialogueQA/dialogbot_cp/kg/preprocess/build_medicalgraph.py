...
sys.path.append('../../..')
from dialogbot_cp.config import host, kg_port, user, password
...
class MedicalGraph:
    ...
    def create_graphnodes(self):
        ...
        rels_recommanddrug, rels_symptom, rels_acompany, rels_category = self._read_nodes()
        self._create_diseases_nodes(disease_infos)
        self._create_node('Drug', Drugs)
        print(len(Drugs))
        self._create_node('Food', Foods)
        print(len(Foods))
        self._create_node('Check', Checks)
        print(len(Checks))
        self._create_node('Department', Departments)
        print(len(Departments))
        self._create_node('Producer', Producers)
        print(len(Producers))
    ...
    def create_graphrels(self):
        ...
        rels_recommandeat, rels_noteat, rels_doeat, rels_department, rels_commonddrug, rels_drug_producer, \
        rels_recommanddrug, rels_symptom, rels_acompany, rels_category = self._read_nodes()
        self._create_relationship('Disease', 'Food', rels_recommandeat, 'recommand_eat', '推荐食谱')
        self._create_relationship('Disease', 'Food', rels_noteat, 'no_eat', '忌吃')
        self._create_relationship('Disease', 'Food', rels_doeat, 'do_eat', '宜吃')
        self._create_relationship('Department', 'Department', rels_department, 'belongs_to', '属于')
        self._create_relationship('Disease', 'Drug', rels_commonddrug, 'common_drug', '常用药品')
        self._create_relationship('Producer', 'Drug', rels_drug_producer, 'drugs_of', '生产药品')
        self._create_relationship('Disease', 'Drug', rels_recommanddrug, 'recommand_drug', '好评药品')
        self._create_relationship('Disease', 'Check', rels_check, 'need_check', '诊断检查')
