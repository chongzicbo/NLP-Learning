...
class MedicalGraph:
    ...
    def collect_medical(self):
        ...
        for item in self.col.find():
            ...
            # 并发症
            data["症状"] = list(
                set([i for i in item["symptom_info"][0] if i[0] not in self.stop_words])
            )
            ...
            for attr, value in data.items():
                ...
                if attr_en in [
                    ...
                    acompany = [
                        i for i in self.cuter.lcut(data_modify[attr_en]) if len(i) > 1
                    ]
                    data_modify[attr_en] = acompany

            try:
                self.db["medical_dict"].insert(data_modify)
                count += 1
                print(count)
