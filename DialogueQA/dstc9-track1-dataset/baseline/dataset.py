...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def _prepare_knowledge(self):
        ...
        for snippet in self.knowledge_docs:
            ...
            tokenized_snippets[key] = tokenized_knowledge[
    ...
    def _create_examples(self):
        ...
        for dialog in tqdm(self.dialogs, disable=self.args.local_rank not in [-1, 0]):
            ...
            if target:
                ...
                knowledge_key = "{}__{}__{}".format(
                    knowledge["domain"], knowledge["entity_id"], knowledge["doc_id"]
                )
...
class KnowledgeSelectionDataset(BaseDataset):
    ...
    def build_input_from_segments(self, knowledge, history):
        ...
        sequence = (
            [sequence[0]]
            + sequence_with_speaker
            + [[self.knowledge_tag] + knowledge + [self.eos]]
        )
        ...
        instance["token_type_ids"] = [
            ...
            for i, s in enumerate(sequence[:-1])
            for _ in s
        ] + [self.knowledge_tag for _ in sequence[-1]]
        instance["mc_token_ids"] = len(instance["input_ids"]) - 1
    ...
    def _shrink_label_cands(self, label, candidates):
        ...
        shrunk_label_cands = random.sample(
            shrunk_label_cands, k=self.args.n_candidates - 1
        )
    ...
    def collate_fn(self, batch):
        ...
        input_ids = torch.tensor(pad_ids(input_ids, self.pad)).view(
            batch_size, n_candidates, -1
        )

        token_type_ids = torch.tensor(pad_ids(token_type_ids, self.pad)).view(
            batch_size, n_candidates, -1
        )
...
class KnowledgeTurnDetectionDataset(BaseDataset):
    ...
    def build_input_from_segments(self, history):
        ...
        sequence = (
            [[self.bos]]
            + history[:-1]
            + [[self.knowledge_tag] + history[-1] + [self.eos]]
        )
        sequence_with_speaker = [
            [self.speaker1 if (len(sequence) - i) % 2 == 0 else self.speaker2] + s
            for i, s in enumerate(sequence[1:])
