...
def evaluate(args, eval_dataset, model, tokenizer, desc="") -> Dict:
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            sampled_output_ids, ground_truth, dialog_id = run_batch_generation_sample(
                args, model, batch, eval_dataset
            )
            sampled_output_text = tokenizer.decode(
                sampled_output_ids, skip_special_tokens=True
            )
    ...
    if args.output_file:
        write_generation_preds(
            eval_dataset.dataset_walker, args.output_file, dialog_ids, all_output_texts
        )
...
def main():
    ...
    result = {}
    if args.local_rank in [-1, 0]:
        eval_dataset = ResponseGenerationEvalDataset(
            dataset_args,
            tokenizer,
        ...
        result = evaluate(
            args, eval_dataset, model, tokenizer, desc=args.eval_desc or "val"
        )
