...
def get_classes(task):
    if task.lower() == "generation":
        ...
        raise ValueError(
            "args.task not in ['generation', 'selection', 'detection'], got %s" % task
        )
...
def evaluate(
    ...
    # multi-gpu evaluate
    if args.n_gpu > 1 and (
        args.task != "selection" or eval_dataset.args.eval_all_snippets
    ):
    ...
    if args.task.lower() == "generation":
        ...
        if args.output_file:
            ...
            write_selection_preds(
                eval_dataset.dataset_walker,
                args.output_file,
                data_infos,
                sorted_pred_ids,
                topk=5,
        ...
        if args.output_file:
            write_detection_preds(
                eval_dataset.dataset_walker, args.output_file, data_infos, all_pred_ids
            )
    else:
        raise ValueError(
...
def main():
    ...
    parser.add_argument(
        "--params_file",
        type=str,
        default="baseline/configs/detection/params.json",
