...
class Metric:
    ...
    def _match(self, ref_knowledge, pred_knowledge):
        ...
        for pred in pred_knowledge:
            ...
            for ref in ref_knowledge:
                if (
                    pred["domain"] == ref["domain"]
                    and pred["entity_id"] == ref["entity_id"]
                    and pred["doc_id"] == ref["doc_id"]
                ):
                    matched = True
        ...
        if ref_obj["target"] is True:
            if hyp_obj["target"] is True:
                ...
                self._selection_mrr5 += self._reciprocal_rank(
                    ref_obj["knowledge"], hyp_obj["knowledge"], 5
                )
                self._selection_r1 += self._recall_at_k(
                    ref_obj["knowledge"], hyp_obj["knowledge"], 1
                )
                self._selection_r5 += self._recall_at_k(
                    ref_obj["knowledge"], hyp_obj["knowledge"], 5
                )
    ...
    def scores(self):
        ...
        selection_mrr5_p, selection_mrr5_r, selection_mrr5_f = self._compute(
            self._selection_mrr5
        )
        selection_r1_p, selection_r1_r, selection_r1_f = self._compute(
            self._selection_r1
        )
        selection_r5_p, selection_r5_r, selection_r5_f = self._compute(
            self._selection_r5
        )

        generation_bleu1_p, generation_bleu1_r, generation_bleu1_f = self._compute(
            self._generation_bleu1
        )
        generation_bleu2_p, generation_bleu2_r, generation_bleu2_f = self._compute(
            self._generation_bleu2
        )
        generation_bleu3_p, generation_bleu3_r, generation_bleu3_f = self._compute(
            self._generation_bleu3
        )
        generation_bleu4_p, generation_bleu4_r, generation_bleu4_f = self._compute(
            self._generation_bleu4
        )
        generation_meteor_p, generation_meteor_r, generation_meteor_f = self._compute(
            self._generation_meteor
        )
        (
            generation_rouge_l_p,
            generation_rouge_l_r,
            generation_rouge_l_f,
        ) = self._compute(self._generation_rouge_l)
        (
            generation_rouge_1_p,
            generation_rouge_1_r,
            generation_rouge_1_f,
        ) = self._compute(self._generation_rouge_1)
        (
            generation_rouge_2_p,
            generation_rouge_2_r,
            generation_rouge_2_f,
        ) = self._compute(self._generation_rouge_2)

        scores = {
            "detection": {"prec": detection_p, "rec": detection_r, "f1": detection_f},
            "selection": {
