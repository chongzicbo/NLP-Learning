...
def main(argv):
    parser = argparse.ArgumentParser(
        description="Check the validity of system outputs."
    )
    ...
    knowledge_reader = KnowledgeReader(
        dataroot=args.dataroot, knowledge_file=args.knowledge_file
    )
    ...
    if len(data) != len(output):
        raise ValueError(
            "the number of instances between ground truth and output does not match"
        )
    ...
    for idx in range(len(output)):
        ...
        # check the additional properties for non-target instances
            ...
            warning_msg.append(
                "On instance[%d]: additional properties for non-target instance" % idx
            )
