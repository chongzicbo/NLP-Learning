...
parser.add_argument(
    "--task_st",
    required=True,
    help="slot filling task: slot_tagger | slot_tagger_with_focus | slot_tagger_with_crf",
)
parser.add_argument(
    "--task_sc",
    required=True,
...
parser.add_argument(
    "--sc_type", default="single_cls_CE", help="single_cls_CE | multi_cls_BCE"
)
parser.add_argument(
    "--st_weight",
    type=float,
...
parser.add_argument(
    "--mini_word_freq", type=int, default=2, help="mini_word_freq in the training data"
)
parser.add_argument("--word_lowercase", action="store_true", help="word lowercase")
parser.add_argument(
    "--word_digit_features", action="store_true", help="using word digit features"
)
parser.add_argument(
    "--bos_eos",
    action="store_true",
...
parser.add_argument(
    "--noStdout", action="store_true", help="Only log to a file; no stdout"
)

parser.add_argument(
    "--testing",
    action="store_true",
    help="Only test your model (default is training && testing)",
)
parser.add_argument(
    "--read_model", required=False, help="Online test: read model from this file"
)
parser.add_argument(
    "--read_vocab", required=False, help="Online test: read input vocab from this file"
)
parser.add_argument("--out_path", required=False, help="Online test: out_path")
parser.add_argument(
    "--read_input_word2vec",
    required=False,
    help="read word embedding from word2vec file",
)
parser.add_argument(
    "--fix_input_word2vec",
    action="store_true",
    help="fix word embedding from word2vec file",
)

parser.add_argument(
    "--emb_size", type=int, default=100, help="word embedding dimension"
)
parser.add_argument(
    "--tag_emb_size", type=int, default=100, help="tag embedding dimension"
)
parser.add_argument(
    "--hidden_size", type=int, default=100, help="hidden layer dimension"
)
...
parser.add_argument(
    "--random_seed", type=int, default=999, help="set initial random seed"
)
...
parser.add_argument(
    "--test_batchSize", type=int, default=0, help="input batch size in decoding"
)
parser.add_argument(
    "--init_weight",
    type=float,
...
parser.add_argument(
    "--max_norm", type=float, default=5, help="threshold of gradient clipping (2-norm)"
)
parser.add_argument(
    "--max_epoch", type=int, default=50, help="max number of epochs to train for"
)
parser.add_argument(
    "--experiment", default="exp", help="Where to store samples and models"
)
...
if opt.deviceId >= 0:
    ...
    if opt.deviceId > 0:
        ...
        opt.deviceId, gpu_name, valid_gpus = gpu_selection.auto_select_gpu()
    logger.info(
        "Valid GPU list: %s ; GPU %d (%s) is auto selected."
...
vocab_config = {
    "mini_word_freq": opt.mini_word_freq,
    "bos_eos": opt.bos_eos,
...
if not opt.testing:
    tag_to_idx, idx_to_tag = vocab_reader.read_vocab_file(
        tag_vocab_dir, bos_eos=opt.enc_dec
    )
    class_to_idx, idx_to_class = vocab_reader.read_vocab_file(
        class_vocab_dir, bos_eos=False
    )
    word_to_idx, idx_to_word = vocab_reader.read_vocab_from_data_file(
        train_data_dir, vocab_config=vocab_config
    )
else:
    tag_to_idx, idx_to_tag = vocab_reader.read_vocab_file(
        opt.read_vocab + ".tag", bos_eos=False, no_pad=True, no_unk=True
    )
    class_to_idx, idx_to_class = vocab_reader.read_vocab_file(
        opt.read_vocab + ".class", bos_eos=False, no_pad=True, no_unk=True
    )
    word_to_idx, idx_to_word = vocab_reader.read_vocab_file(
        opt.read_vocab + ".in", bos_eos=False, no_pad=True, no_unk=True
    )

if not opt.testing and opt.read_input_word2vec:
    # pretrained-embedding initialization for training
    ext_word_to_idx, ext_word_emb = read_wordEmb.read_word2vec_inText(
        opt.read_input_word2vec, opt.device
    )
...
logger.info(
    "Vocab size: %s %s %s" % (len(word_to_idx), len(tag_to_idx), len(class_to_idx))
)
if not opt.testing:
    ...
    vocab_reader.save_vocab(
        idx_to_class, os.path.join(exp_path, opt.save_vocab + ".class")
    )

if not opt.testing:
    train_feats, train_tags, train_class = data_reader.read_seqtag_data_with_class(
        train_data_dir,
        word_to_idx,
        tag_to_idx,
        class_to_idx,
    ...
    valid_feats, valid_tags, valid_class = data_reader.read_seqtag_data_with_class(
        valid_data_dir,
        word_to_idx,
        tag_to_idx,
        class_to_idx,
    ...
    test_feats, test_tags, test_class = data_reader.read_seqtag_data_with_class(
        test_data_dir,
        word_to_idx,
        tag_to_idx,
        class_to_idx,
...
if opt.task_sc == "2tails":
    ...
    model_class = snt_classifier.sntClassifier_hiddenPooling(
        opt.hidden_size,
        len(class_to_idx),
        bidirectional=opt.bidirectional,
        num_layers=opt.num_layers,
    ...
    model_class = snt_classifier.sntClassifier_hiddenCNN(
        opt.hidden_size,
        len(class_to_idx),
        bidirectional=opt.bidirectional,
        num_layers=opt.num_layers,
    ...
    model_class = snt_classifier.sntClassifier_hiddenAttention(
        opt.hidden_size,
        len(class_to_idx),
        bidirectional=opt.bidirectional,
        num_layers=opt.num_layers,
    ...
    if opt.read_input_word2vec:
        ...
        for word in word_to_idx:
            if word in ext_word_to_idx:
                model_tag.word_embeddings.weight.data[word_to_idx[word]] = ext_word_emb[
                    ext_word_to_idx[word]
                ]
            else:
                word_out_of_pretrained_emb_count += 1
        logger.info(
            "${word_out_of_pretrained_emb_count} is %s !"
            % (word_out_of_pretrained_emb_count)
        )
        if opt.fix_input_word2vec:
...
)  # must be list, otherwise clip_grad_norm_ will be invalid
if opt.optim.lower() == "sgd":
    optimizer = optim.SGD(params, lr=opt.lr)
elif opt.optim.lower() == "adam":
    optimizer = optim.Adam(
        params, lr=opt.lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=0
    )  # (beta1, beta2)
...
def decode(data_feats, data_tags, data_class, output_path):
        ...
        for j in range(0, len(data_index), opt.test_batchSize):
            if opt.testing:
                (
                    ...
                    lens,
                    line_nums,
                ) = data_reader.get_minibatch_with_class(
                    data_feats,
                    data_tags,
                    data_class,
                    word_to_idx,
            ...
            if opt.word_digit_features:
                word_seqs = [
                    [idx_to_word[w_idx] for w_idx in word_seq]
                    for word_seq in inputs.data.cpu().numpy()
                ]
            ...
            if opt.enc_dec:
                ...
                if opt.greed_decoding:
                    ...
                    tag_loss = tag_loss_function(
                        tag_scores_1best.contiguous().view(-1, len(tag_to_idx)),
                    ...
                    top_pred_slots = [
                        [item[0].item() for item in seq] for seq in top_path_slots
                    ]
                ...
                if opt.word_digit_features:
                    crf_feats, encoder_info = model_tag._get_lstm_features(
                        inputs, lens, with_snt_classifier=True, extFeats=ext_features
                    )
                else:
                    crf_feats, encoder_info = model_tag._get_lstm_features(
                ...
                if opt.word_digit_features:
                    tag_scores, encoder_info = model_tag(
                        inputs, lens, with_snt_classifier=True, extFeats=ext_features
                    )
                else:
                    tag_scores, encoder_info = model_tag(
                ...
                tag_loss = tag_loss_function(
                    tag_scores.contiguous().view(-1, len(tag_to_idx)), tags.view(-1)
                )
            ...
            if opt.task_sc:
                ...
                losses.append(
                    [tag_loss.item() / sum(lens), class_loss.item() / len(lens)]
                )
            else:
            ...
            for idx, pred_line in enumerate(top_pred_slots):
                ...
                word_tag_line = [
                    input_line[_idx] + ":" + lab_seq[_idx] + ":" + pred_seq[_idx]
                ...
                if opt.task_sc:
                    if opt.multiClass:
                        pred_classes = [
                            idx_to_class[i]
                            for i, p in enumerate(snt_probs[idx])
                            if p > 0.5
                        ]
    ...
    return (
        ...
        r,
        f,
        0 if 2 * TP2 + FN2 + FP2 == 0 else 100 * 2 * TP2 / (2 * TP2 + FN2 + FP2),
    )
...
if not opt.testing:
    ...
    for i in range(opt.max_epoch):
        ...
        piece_sentences = (
            opt.batchSize
            if int(nsentences * 0.1 / opt.batchSize) == 0
            else int(nsentences * 0.1 / opt.batchSize) * opt.batchSize
        )
        for j in range(0, nsentences, opt.batchSize):
            ...
            if opt.enc_dec:
                if opt.word_digit_features:
                    ...
                    tag_scores, encoder_info = model_tag(
                        inputs, tags[:, :-1], lens, with_snt_classifier=True
                    )
                tag_loss = tag_loss_function(
            ...
            if j % piece_sentences == 0:
                print(
                    ...
                    % (i, (j + opt.batchSize) * 100.0 / nsentences),
        ...
        # Evaluation
        start_time = time.time()
        loss_val, p_val, r_val, f_val, cf_val = decode(
            valid_feats["data"],
            valid_tags["data"],
            valid_class["data"],
        ...
        logger.info(
            "Validation:\tEpoch : %d\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f "
            % (i, time.time() - start_time, loss_val[0], loss_val[1], f_val, cf_val)
        )
        start_time = time.time()
        loss_te, p_te, r_te, f_te, cf_te = decode(
            test_feats["data"],
            test_tags["data"],
            test_class["data"],
        ...
        logger.info(
            "Evaluation:\tEpoch : %d\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f "
            % (i, time.time() - start_time, loss_te[0], loss_te[1], f_te, cf_te)
        )
        ...
        if best_f1 < val_f1_score:
            ...
            if opt.task_sc:
                model_class.save_model(
                    os.path.join(exp_path, opt.save_model + ".class")
                )
            best_f1 = val_f1_score
            logger.info(
                "NEW BEST:\tEpoch : %d\tbest valid F1 : %.2f, cls-F1 : %.2f;\ttest F1 : %.2f, cls-F1 : %.2f"
                % (i, f_val, cf_val, f_te, cf_te)
            )
            best_result["iter"] = i
            best_result["vf1"], best_result["vcf1"], best_result["vce"] = (
                f_val,
                cf_val,
                loss_val,
            )
            best_result["tf1"], best_result["tcf1"], best_result["tce"] = (
                f_te,
                cf_te,
                loss_te,
            )
    logger.info(
        "BEST RESULT: \tEpoch : %d\tbest valid (Loss: (%.2f, %.2f) F1 : %.2f cls-F1 : %.2f)\tbest test (Loss: (%.2f, %.2f) F1 : %.2f cls-F1 : %.2f) "
        % (
            best_result["iter"],
            best_result["vce"][0],
            best_result["vce"][1],
    ...
    logger.info(
        "Validation:\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f "
        % (time.time() - start_time, loss_val[0], loss_val[1], f_val, cf_val)
    )
    start_time = time.time()
    loss_te, p_te, r_te, f_te, cf_te = decode(
        test_feats["data"],
    ...
    logger.info(
        "Evaluation:\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f "
        % (time.time() - start_time, loss_te[0], loss_te[1], f_te, cf_te)
    )
