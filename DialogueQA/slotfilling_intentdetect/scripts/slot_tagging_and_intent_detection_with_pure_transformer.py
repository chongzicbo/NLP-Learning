...
parser.add_argument(
    "--task_sc", required=True, help="intent detection task: none | CLS | max | CLS_max"
)
parser.add_argument(
...
model_tag_and_class = joint_transformer.Transformers_joint_slot_and_intent(
    opt.pretrained_model_type,
    pretrained_model,
    len(tag_to_idx),
    len(class_to_idx),
    dropout=opt.dropout,
...
if opt.optim.lower() == "sgd":
    ...
    print(
        "total steps %d; warmup_proportion %.2f"
        % (num_train_optimization_steps, opt.warmup_proportion)
    )
    ...
    with open(output_path, "w") as f, torch.no_grad():
        for j in range(0, len(data_index), opt.test_batchSize):
            ...
            if opt.task_st == "NN":
                ...
                tag_path_scores, tag_path = model_tag_and_class.crf_viterbi_decode(
                    crf_feats, masks
                )
                tag_loss = model_tag_and_class.crf_neg_log_likelihood(
                    crf_feats, masks, tags
                )
    ...
    for i in range(opt.max_epoch):
        ...
        for j in range(0, nsentences, opt.batchSize // opt.gradient_accumulation_steps):
            (
                ...
                train_data_index,
                j,
                opt.batchSize // opt.gradient_accumulation_steps,
                add_start_end=opt.bos_eos,
                multiClass=opt.multiClass,
                enc_dec_focus=False,
                device=opt.device,
