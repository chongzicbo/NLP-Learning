...
parser.add_argument('--task_st', default="slot_tagger_with_focus",
                    help='slot filling task: slot_tagger | slot_tagger_with_focus | slot_tagger_with_crf')
parser.add_argument('--task_sc', default="hiddenAttention",
...
parser.add_argument('--dataroot', default="/mnt/e/opensource_data/对话问答/意图识别和槽填充/data/snips",
...
parser.add_argument('--pretrained_model_name', default="xlnet-base-cased",
...
parser.add_argument('--dropout', type=float, default=0.1, help='dropout rate at each non-recurrent layer')
...
if not opt.testing:
    train_feats, train_tags, train_class = data_reader.read_seqtag_data_with_class(train_data_dir, tag_to_idx,
                                                                                   class_to_idx,
                                                                                   multiClass=opt.multiClass,
                                                                                   lowercase=opt.word_lowercase)
    valid_feats, valid_tags, valid_class = data_reader.read_seqtag_data_with_class(valid_data_dir, tag_to_idx,
                                                                                   class_to_idx,
                                                                                   multiClass=opt.multiClass,
                                                                                   keep_order=opt.testing,
                                                                                   lowercase=opt.word_lowercase)
    test_feats, test_tags, test_class = data_reader.read_seqtag_data_with_class(test_data_dir, tag_to_idx, class_to_idx,
                                                                                multiClass=opt.multiClass,
                                                                                keep_order=opt.testing,
                                                                                lowercase=opt.word_lowercase)
else:
...
if opt.task_st == 'slot_tagger':
    model_tag = slot_tagger.LSTMTagger(opt.emb_size, opt.hidden_size, None, len(tag_to_idx),
                                       bidirectional=opt.bidirectional, num_layers=opt.num_layers, dropout=opt.dropout,
                                       device=opt.device, pretrained_model=pretrained_model,
    ...
    model_tag = slot_tagger_with_focus.LSTMTagger_focus(opt.emb_size, opt.tag_emb_size, opt.hidden_size, None,
                                                        len(tag_to_idx), bidirectional=opt.bidirectional,
                                                        num_layers=opt.num_layers, dropout=opt.dropout,
                                                        device=opt.device, pretrained_model=pretrained_model,
                                                        pretrained_model_type=opt.pretrained_model_type,
                                                        fix_pretrained_model=opt.fix_pretrained_model)
elif opt.task_st == 'slot_tagger_with_crf':
    model_tag = slot_tagger_with_crf.LSTMTagger_CRF(opt.emb_size, opt.hidden_size, None, len(tag_to_idx),
...
if opt.optim.lower() == 'adam':
    ...
    optimizer = BertAdam(optimizer_grouped_parameters, lr=opt.lr, warmup=opt.warmup_proportion,
    ...
    num_train_optimization_steps = len(train_feats['data']) // opt.batchSize * opt.max_epoch
    optimizer = AdamW(optimizer_grouped_parameters, lr=opt.lr,
                      correct_bias=False)  # To reproduce BertAdam specific behavior set correct_bias=False
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=int(
        opt.warmup_proportion * num_train_optimization_steps),
        ...
        for j in range(0, len(data_index), opt.test_batchSize):
            if opt.testing:
                words, tags, raw_tags, classes, raw_classes, lens, line_nums = data_reader.get_minibatch_with_class(
                    data_feats, data_tags, data_class, tag_to_idx, class_to_idx, data_index, j, opt.test_batchSize,
                    add_start_end=opt.bos_eos, multiClass=opt.multiClass, keep_order=opt.testing,
                    enc_dec_focus=opt.enc_dec, device=opt.device)
            else:
                words, tags, raw_tags, classes, raw_classes, lens = data_reader.get_minibatch_with_class(data_feats,
                                                                                                         data_tags,
                                                                                                         data_class,
                                                                                                         tag_to_idx,
                                                                                                         class_to_idx,
            ...
            inputs = prepare_inputs_for_bert_xlnet(words, lens, tokenizer,
                                                   ...
                                                   cls_token_segment_id=2 if opt.pretrained_model_type in [
                                                       'xlnet'] else 0,
                                                   pad_on_left=bool(opt.pretrained_model_type in ['xlnet']),
                                                   # pad on the left for xlnet
                                                   pad_token_segment_id=4 if opt.pretrained_model_type in [
                                                       'xlnet'] else 0,
                                                   device=opt.device)
            ...
            if opt.enc_dec:
                ...
                if opt.greed_decoding:
                    tag_scores_1best, outputs_1best, encoder_info = model_tag.decode_greed(inputs, tags[:, 0:1], lens,
...
if not opt.testing:
    ...
    for i in range(opt.max_epoch):
        ...
        for j in range(0, nsentences, opt.batchSize):
            words, tags, raw_tags, classes, raw_classes, lens = data_reader.get_minibatch_with_class(
                train_feats['data'], train_tags['data'], train_class['data'], tag_to_idx, class_to_idx,
            ...
            if opt.task_sc:
                ...
                class_loss = class_loss_function(class_scores, classes)
                losses.append([tag_loss.item() / sum(lens), class_loss.item() / len(lens)])
                total_loss = opt.st_weight * tag_loss + (1 - opt.st_weight) * class_loss  # 实体识别的损失+分类的损失
            else:
                losses.append([tag_loss.item() / sum(lens), 0])
                total_loss = tag_loss
            total_loss.backward()
        ...
        # Evaluation
        start_time = time.time()
        loss_val, p_val, r_val, f_val, cf_val = decode(valid_feats['data'], valid_tags['data'], valid_class['data'],
                                                       os.path.join(exp_path, 'valid.iter' + str(i)))
        logger.info('Validation:\tEpoch : %d\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f ' % (
            i, time.time() - start_time, loss_val[0], loss_val[1], f_val, cf_val))
        start_time = time.time()
        loss_te, p_te, r_te, f_te, cf_te = decode(test_feats['data'], test_tags['data'], test_class['data'],
                                                  os.path.join(exp_path, 'test.iter' + str(i)))
        logger.info('Evaluation:\tEpoch : %d\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f ' % (
        ...
        if best_f1 < val_f1_score:
            ...
            logger.info('NEW BEST:\tEpoch : %d\tbest valid F1 : %.2f, cls-F1 : %.2f;\ttest F1 : %.2f, cls-F1 : %.2f' % (
    ...
    logger.info(
        'BEST RESULT: \tEpoch : %d\tbest valid (Loss: (%.2f, %.2f) F1 : %.2f cls-F1 : %.2f)\tbest test (Loss: (%.2f, %.2f) F1 : %.2f cls-F1 : %.2f) ' % (
            best_result['iter'], best_result['vce'][0], best_result['vce'][1], best_result['vf1'], best_result['vcf1'],
            best_result['tce'][0], best_result['tce'][1], best_result['tf1'], best_result['tcf1']))
else:
    ...
    logger.info('Validation:\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f ' % (
        time.time() - start_time, loss_val[0], loss_val[1], f_val, cf_val))
    start_time = time.time()
    loss_te, p_te, r_te, f_te, cf_te = decode(test_feats['data'], test_tags['data'], test_class['data'],
                                              os.path.join(exp_path, 'test.eval'))
    logger.info('Evaluation:\tTime : %.4fs\tLoss : (%.2f, %.2f)\tFscore : %.2f\tcls-F1 : %.2f ' % (
