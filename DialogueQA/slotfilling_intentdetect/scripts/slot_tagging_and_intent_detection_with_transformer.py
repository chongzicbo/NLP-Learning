...
@author: chengbo
@software: PyCharm
@file: slot_tagging_and_intent_detection_with_transformer.py
@time: 2022/9/7 20:51
"""
import argparse
import random
...
parser.add_argument("--dataset", default="snips", help="atis-2 | snips")
parser.add_argument(
    "--dataroot",
    default="/mnt/e/opensource_data/对话问答/意图识别和槽填充/data/snips",
...
parser.add_argument("--pretrained_model_type", default="xlnet", help="bert, xlnet")
parser.add_argument(
...
parser.add_argument(
    "--fix_pretrained_model",
    action="store_true",
    help="fix pretrained (bert/xlnet) model",
)
...
if opt.optim.lower() == "adam":
    ...
    num_train_optimization_steps = (
        len(train_feats["data"]) // opt.batchSize * opt.max_epoch
    )
    optimizer = BertAdam(
        optimizer_grouped_parameters,
        lr=opt.lr,
    ...
    with open(output_path, "w") as f:
        for j in range(0, len(data_index), opt.test_batchSize):
            ...
            inputs = prepare_inputs_for_bert_xlnet(
                ...
                pad_on_left=bool(opt.pretrained_model_type in ["xlnet"]),
            ...
            if opt.enc_dec:
                ...
                if opt.greed_decoding:
                    ...
                    (
                        ...
                        inputs, lens, beam_size, tag_to_idx, with_snt_classifier=True
                    )
                    top_pred_slots = [
    ...
    for i in range(opt.max_epoch):
        ...
        for j in range(0, nsentences, opt.batchSize):
            ...
            if opt.task_sc:
                ...
                total_loss = (
                    opt.st_weight * tag_loss + (1 - opt.st_weight) * class_loss
                )  # 实体识别的损失+分类的损失
            else:
                losses.append([tag_loss.item() / sum(lens), 0])
                total_loss = tag_loss
