...
class elmo_embeddings:
    def __init__(self, options_file, weight_file, device=None):
        self._elmo_lstm = _ElmoBiLm(
            options_file, weight_file, requires_grad=False, vocab_to_cache=None
        )
    ...
    def forward(self, inputs):
        ...
        assert self.output_dim * len(
            layer_activations
        ) == word_embedding_and_hiddens.size(-1)
            ...
            for i in range(1, len(layer_activations)):
                out_representations.append(
                    processed_representation[
                        :, :, self.output_dim * i : self.output_dim * (i + 1)
                    ]
                )
...
if __name__ == "__main__":
    ...
    with open(args.output_word2vec, "w") as out_file:
        out_file.write(
            str(len(word_vocab)) + " " + str(to_get_elmo_embeddings.output_dim) + "\n"
        )
        for i in range(0, len(word_vocab), batch_size):
            sentences = [[word] for word in word_vocab[i : i + batch_size]]
            character_ids = batch_to_ids(sentences)
            word_embeddings = to_get_elmo_embeddings.forward(character_ids)[
