...
if __name__ == "__main__":
    ...
    (
        correct_sentence_slots,
        correct_sentence_intents,
        correct_sentence,
        sentence_number,
    ) = (0.0, 0.0, 0.0, 0.0)
    all_slots = {}
    for line in file:
        ...
        if len(tmps) > 1:
            ...
            intent_label_items = (
                set(intent_label.split(";")) if intent_label != "" else set()
            )
            intent_pred_items = (
                set(intent_pred.split(";")) if intent_pred != "" else set()
            )
        ...
        if label_chunks != pred_chunks and opt.print_log:
            print(
                " ".join(
                    [
                        word if label == "O" else word + ":" + label
    ...
    table = prettytable.PrettyTable(
        ["Metric", "TP", "FN", "FP", "Prec.", "Recall", "F1-score", "Sentence Acc"]
    )
    ## 自定义表格输出样式
    ### 设定左对齐
    table.align = "l"
    ...
    if TP == 0:
        table.add_row(
            (
                ...
                0,
                0,
                100 * correct_sentence_slots / sentence_number,
            )
        )
    else:
        table.add_row(
    ...
    if TP2 != 0:
        table.add_row(
            (
                ...
                100 * TP2 / (TP2 + FP2),
                100 * TP2 / (TP2 + FN2),
                100 * 2 * TP2 / (2 * TP2 + FN2 + FP2),
                100 * correct_sentence_intents / sentence_number,
            )
        )
        table.add_row(
            (
                ...
                "-",
                "-",
                100 * correct_sentence / sentence_number,
            )
        )
    table.add_row(("-", "-", "-", "-", "-", "-", "-", "-"))
    all_F1 = []
    for slot, _ in sorted(
        all_slots.items(),
        key=lambda kv: (kv[1]["FN"] + kv[1]["TP"], kv[0]),
        reverse=True,
    ):
