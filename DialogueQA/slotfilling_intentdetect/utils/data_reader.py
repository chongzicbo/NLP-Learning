    ...
    with open(data_path, "r") as f:
        for ind, line in enumerate(f):
            ...
            for item in slot_tag_line.split(" "):
                ...
                tag_seq.append(
                    tag2idx[tag] if tag in tag2idx else (tag2idx["<unk>"], tag)
                )
            if keep_order:
            ...
            if multiClass:
                if class_name == "":
                    ...
                    class_labels.append(
                        [class2idx[val] for val in class_name.split(";")]
                    )
            else:
                if ";" not in class_name:
                    ...
                    class_labels.append(
                        (class2idx[class_name.split(";")[0]], class_name.split(";"))
...
def get_minibatch_with_class(
    ...
    """Prepare minibatch."""
    input_seqs = [
        input_seqs[idx] for idx in train_data_indx[index : index + batch_size]
    ]
    tag_seqs = [tag_seqs[idx] for idx in train_data_indx[index : index + batch_size]]
    class_labels = [
        class_labels[idx] for idx in train_data_indx[index : index + batch_size]
    ]
    if add_start_end:
        input_seqs = [
            [word2idx["<s>"]] + line + [word2idx["</s>"]] for line in input_seqs
        ]
    ...
    raw_tags = [
        [item[1] if type(item) in {list, tuple} else item for item in tag]
        for seq, tag, cls in data_mb
    ]
    data_mb = [
        (seq, [item[0] if type(item) in {list, tuple} else item for item in tag], cls)
        for seq, tag, cls in data_mb
    ]
    if keep_order:
    ...
    max_len = max(lens)
    input_idxs = [
        seq + [word2idx["<pad>"]] * (max_len - len(seq)) for seq, _, _ in data_mb
    ]
    input_idxs = torch.tensor(input_idxs, dtype=torch.long, device=device)

    if not enc_dec_focus:
        tag_idxs = [
            seq + [tag2idx["<pad>"]] * (max_len - len(seq)) for _, seq, _ in data_mb
        ]
    else:
        tag_idxs = [
    ...
    if multiClass:
        ...
        raw_classes = [
            class_label[1] if type(class_label) in {list, tuple} else class_label
            for _, _, class_label in data_mb
        ]
        class_idxs = [
            class_label[0] if type(class_label) in {list, tuple} else class_label
            for _, _, class_label in data_mb
        ]
