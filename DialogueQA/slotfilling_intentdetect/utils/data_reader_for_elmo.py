...
def read_seqtag_data_with_class(data_path, tag2idx, class2idx, separator=':', multiClass=False, keep_order=False,
                                lowercase=False):
    '''
        ...
        for ind, line in enumerate(f):
            ...
            if multiClass:
                ...
                if ';' not in class_name:
                    ...
                    class_labels.append((class2idx[class_name.split(';')[0]],
                                         class_name.split(';')))  # get the first class for training
...
def get_minibatch_with_class(input_seqs, tag_seqs, class_labels, tag2idx, class2idx, train_data_indx, index, batch_size,
                             add_start_end=False, multiClass=False, keep_order=False, enc_dec_focus=False, device=None):
    """Prepare minibatch."""
    ...
    data_mb.sort(key=lambda x: len(x[0]), reverse=True)  # 按照文本长度进行排序 # sorted for pad setence
