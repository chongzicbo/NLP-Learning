...
def read_vocab_file(
    vocab_path, bos_eos=False, no_pad=False, no_unk=False, separator=":"
):
...
def construct_vocab(input_seqs, vocab_config={"mini_word_freq": 1, "bos_eos": False}):
    ...
    sorted_word2id = sorted(vocab.items(), key=operator.itemgetter(1), reverse=True)

    sorted_words = [
        x[0] for x in sorted_word2id if x[1] >= vocab_config["mini_word_freq"]
    ]
...
def read_vocab_from_data_file(
    data_path,
    vocab_config={"mini_word_freq": 1, "bos_eos": False, "lowercase": False},
    with_tag=True,
    separator=":",
):
    """
