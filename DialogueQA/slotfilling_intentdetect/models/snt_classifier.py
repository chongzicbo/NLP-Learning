...
class sntClassifier_2tails(nn.Module):
    ...
    def __init__(
        ...
        if self.hidden_layers == 1:
            self.hidden2class = nn.Linear(
                self.num_directions * self.hidden_dim, self.class_size
            )
        else:
    ...
    def forward(self, packed_h_t_c_t, masked_output=None):
        index_slices = (
            [self.num_layers * 2 - 2, self.num_layers * 2 - 1]
            if self.bidirectional
            else [self.num_layers * 1 - 1]
        )
...
class sntClassifier_hiddenPooling(nn.Module):
    ...
    def forward(self, inputs, masked_output=None):
        ...
        if self.pooling == "mean":
            len_sum = torch.tensor(
                lens, dtype=torch.float, device=self.device
            ).unsqueeze(1)
            ...
            max_len = max(lens)
            mask = [([0] * l) + ([1] * (max_len - l)) for l in lens]
...
class sntClassifier_hiddenCNN(nn.Module):
    ...
    def forward(self, inputs, masked_output=None):
        ...
        if not self.conv2d:
            ...
            conv_hiddens_pool = (
                conv_hiddens_pool_1  # + conv_hiddens_pool_2 + conv_hiddens_pool_3
            )
        else:
...
class sntClassifier_hiddenAttention(nn.Module):
    ...
    def __init__(
        ...
        self.Ua = nn.Conv1d(
            self.num_directions * self.hidden_dim, self.hidden_dim, 1, bias=False
        )
