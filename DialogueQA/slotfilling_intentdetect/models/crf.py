...
class CRF(nn.Module):
    ...
    def _calculate_alg(self, feats, mask):
        ...
        (
            _,
            inivalues,
        ) = seq_iter.__next__()  # bat_size * from_target_size * to_target_size
    ...
    def _viterbi_decode_nbest(self, feats, mask, nbest):
        ...
        pad_zero = torch.zeros(batch_size, tag_size, nbest).to(
        ...
        decode_idx = torch.zeros(
            seq_len, batch_size, nbest, dtype=torch.long, device=self.device
        )
