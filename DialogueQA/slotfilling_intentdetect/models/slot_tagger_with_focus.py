...
class LSTMTagger_focus(nn.Module):
    ...
    def __init__(self, embedding_dim, tag_embedding_dim, hidden_dim, vocab_size, tagset_size, bidirectional=True,
                 num_layers=1, dropout=0., device=None, extFeats_dim=None, decoder_tied=False, elmo_model=None,
                 pretrained_model=None, pretrained_model_type=None, fix_pretrained_model=False):
        """Initialize model."""
        ...
        self.encoder = nn.LSTM(self.embedding_dim + self.append_feature_dim, self.hidden_dim,
                               num_layers=self.num_layers, bidirectional=self.bidirectional, batch_first=True,
                               dropout=self.dropout)

        self.decoder = nn.LSTM(self.tag_embedding_dim + self.num_directions * self.hidden_dim, self.hidden_dim,
    ...
    def get_token_embeddings(self, word_seqs, lengths):
        if self.elmo_model and self.pretrained_model:
            ...
            tokens, segments, selects, copies, attention_mask = word_seqs['transformer']['tokens'], \
            ...
            if self.fix_pretrained_model:
                ...
                used_hiddens = torch.cat([hiddens.unsqueeze(3) for hiddens in
                                          pretrained_all_hiddens[- self.number_of_last_hiddens_of_pretrained:]], dim=-1)
                pretrained_top_hiddens = self.weighted_scores_of_last_hiddens(used_hiddens).squeeze(3)
            else:
                pretrained_top_hiddens = outputs[0]
            batch_size, pretrained_seq_length, hidden_size = pretrained_top_hiddens.size(
                0), pretrained_top_hiddens.size(1), pretrained_top_hiddens.size(2)
            ...
            pretrained_embeds = pretrained_embeds.index_copy_(0, copies, chosen_encoder_hiddens).view(len(lengths),
                                                                                                      max(lengths), -1)
            ...
            tokens, segments, selects, copies, attention_mask = word_seqs['tokens'], word_seqs['segments'], word_seqs[
                'selects'], word_seqs['copies'], word_seqs['mask']
            outputs = self.pretrained_model(tokens, token_type_ids=segments, attention_mask=attention_mask)
            if self.fix_pretrained_model:
                pretrained_all_hiddens = outputs[2]
                used_hiddens = torch.cat([hiddens.unsqueeze(3) for hiddens in
    ...
    def forward(self, word_seqs, tag_seqs, lengths, extFeats=None, with_snt_classifier=False, masked_output=None):
        ...
        packed_word_embeds = rnn_utils.pack_padded_sequence(concat_input, lengths, batch_first=True)  # 对padding后的序列进行压缩
        ...
        tag_lstm_out_reshape = tag_lstm_out.contiguous().view(tag_lstm_out.size(0) * tag_lstm_out.size(1),
                                                              tag_lstm_out.size(2))
    ...
    def decode_greed(self, word_seqs, init_tags, lengths, extFeats=None, with_snt_classifier=False, masked_output=None):
        ...
        for i in range(max_length):
            ...
            decode_inputs = torch.cat((self.dropout_layer(word_lstm_out[:, i:i + 1]), tag_embeds),
                                      2)  # bsize x 1 x insize
            tag_lstm_out, (dec_h_t, dec_c_t) = self.decoder(decode_inputs,
    ...
    def decode_beam_search(self, word_seqs, lengths, beam_size, tag2idx, extFeats=None, with_snt_classifier=False,
                           masked_output=None):
        minibatch_size = len(lengths)  # word_seqs.size(0) if self.encoder.batch_first else word_seqs.size(1)
        max_length = max(lengths)  # word_seqs.size(1) if self.encoder.batch_first else word_seqs.size(0)
        # encoder
        ...
        top_dec_h_t, top_dec_c_t = [0] * minibatch_size, [0] * minibatch_size
        for i in range(max_length):
            last_tags = torch.stack([b.get_current_state() for b in beam if not b.done]).t().contiguous().view(-1,
            ...
            tag_lstm_out, (dec_h_t, dec_c_t) = self.decoder(decode_inputs, (
                h_t, c_t))  # (batch*beam) x 1 x insize => (batch*beam) x 1 x hsize
            ...
            word_lstm_out = update_active(word_lstm_out.transpose(0, 1),
