...
class LSTMTagger_focus(nn.Module):
    def __init__(
        ...
        if self.fix_pretrained_model:
            ...
            self.weighted_scores_of_last_hiddens = nn.Linear(
                self.number_of_last_hiddens_of_pretrained, 1, bias=False
            )
        ...
        if self.elmo_model and self.pretrained_model:
            self.embedding_dim = (
                self.elmo_model.get_output_dim()
        ...
        if self.extFeats_dim:
            ...
            self.extFeats_linear = nn.Linear(
                self.append_feature_dim, self.append_feature_dim
            )
        else:
        ...
        self.decoder = nn.LSTM(
            self.tag_embedding_dim + self.num_directions * self.hidden_dim,
            self.hidden_dim,
            num_layers=self.num_layers,
            bidirectional=False,
            batch_first=True,
    ...
    def init_weights(self, initrange=0.2):
        ...
        if self.fix_pretrained_model:
            self.weighted_scores_of_last_hiddens.weight.data.uniform_(
                -initrange, initrange
            )
        ...
        for weight in self.encoder.parameters():
            weight.data.uniform_(-initrange, initrange)  # 权重正态分布初始化
    ...
    def get_token_embeddings(self, word_seqs, lengths):
        if self.elmo_model and self.pretrained_model:
            ...
            tokens, segments, selects, copies, attention_mask = (
                word_seqs["transformer"]["tokens"],
                word_seqs["transformer"]["segments"],
                word_seqs["transformer"]["selects"],
                word_seqs["transformer"]["copies"],
                word_seqs["transformer"]["mask"],
            )
            ...
            if self.fix_pretrained_model:
                ...
                used_hiddens = torch.cat(
                    [
                        hiddens.unsqueeze(3)
                        for hiddens in pretrained_all_hiddens[
                            -self.number_of_last_hiddens_of_pretrained :
                        ]
                    ],
                    dim=-1,
                )
                pretrained_top_hiddens = self.weighted_scores_of_last_hiddens(
                    used_hiddens
                ).squeeze(3)
            else:
                pretrained_top_hiddens = outputs[0]
            batch_size, pretrained_seq_length, hidden_size = (
                pretrained_top_hiddens.size(0),
                pretrained_top_hiddens.size(1),
                pretrained_top_hiddens.size(2),
            )
            chosen_encoder_hiddens = pretrained_top_hiddens.view(
                -1, hidden_size
            ).index_select(0, selects)
            pretrained_embeds = torch.zeros(
                len(lengths) * max(lengths), hidden_size, device=self.device
            )
            pretrained_embeds = pretrained_embeds.index_copy_(
                0, copies, chosen_encoder_hiddens
            ).view(len(lengths), max(lengths), -1)
            ...
            embeds = embeds.index_copy_(0, copies, chosen_encoder_hiddens).view(
    ...
    def forward(
        ...
        if type(extFeats) != type(None):
            ...
            concat_input = embeds
        concat_input = self.dropout_layer(concat_input)  # 对embedding进行dropout
        packed_word_embeds = rnn_utils.pack_padded_sequence(
            concat_input, lengths, batch_first=True
        )  # 对padding后的序列进行压缩
        # xlnet输出的embedding作为LSTM的输入
        ...
        if self.bidirectional:
            ...
            index_slices = torch.tensor(
                index_slices, dtype=torch.long, device=self.device
            )
        ...
        packed_decode_inputs = rnn_utils.pack_padded_sequence(
            decode_inputs, lengths, batch_first=True
        )
        packed_tag_lstm_out, (dec_h_t, dec_c_t) = self.decoder(
            packed_decode_inputs, (h_t, c_t)
        )  # bsize x seqlen x dim
        tag_lstm_out, unpacked_len = rnn_utils.pad_packed_sequence(
            packed_tag_lstm_out, batch_first=True
        )

        tag_lstm_out_reshape = tag_lstm_out.contiguous().view(
            tag_lstm_out.size(0) * tag_lstm_out.size(1), tag_lstm_out.size(2)
        )
        tag_space = self.hidden2tag(self.dropout_layer(tag_lstm_out_reshape))
        tag_scores = F.log_softmax(tag_space, dim=1)
        tag_scores = tag_scores.view(
            tag_lstm_out.size(0), tag_lstm_out.size(1), tag_space.size(1)
        )
    ...
    def decode_greed(
        ...
        for i in range(max_length):
            ...
            decode_inputs = torch.cat(
                (self.dropout_layer(word_lstm_out[:, i : i + 1]), tag_embeds), 2
            )  # bsize x 1 x insize
            tag_lstm_out, (dec_h_t, dec_c_t) = self.decoder(
                decode_inputs, (h_t, c_t)
            )  # bsize x 1 x insize => bsize x 1 x hsize
        ...
        if with_snt_classifier:
            return (
                top_path_tag_scores,
                top_path,
                ((enc_h_t, enc_c_t), word_lstm_out, lengths),
            )
        else:
    ...
    def decode_beam_search(
        ...
        beam = [
            Beam(beam_size, tag2idx, device=self.device) for k in range(minibatch_size)
        ]
        ...
        for i in range(max_length):
            last_tags = (
                ...
                .contiguous()
                .view(-1, 1)
            )  # after t() -> beam_size * batch_size
            last_tags = last_tags.to(self.device)
            tag_embeds = self.dropout_layer(self.tag_embeddings(last_tags))
            decode_inputs = torch.cat(
                (self.dropout_layer(word_lstm_out[:, i : i + 1]), tag_embeds), 2
            )  # (batch*beam) x 1 x insize
            tag_lstm_out, (dec_h_t, dec_c_t) = self.decoder(
                decode_inputs, (h_t, c_t)
            )  # (batch*beam) x 1 x insize => (batch*beam) x 1 x hsize
            ...
            active_idx = torch.tensor(
                [batch_idx[k] for k in active], dtype=torch.long, device=self.device
            )
            ...
            def update_active(t, hidden_dim):
                ...
                new_size[-2] = (
                    new_size[-2] * len(active_idx) // remaining_sents
                )  # beam*len(active_idx)
            ...
            word_lstm_out = update_active(
                word_lstm_out.transpose(0, 1), self.num_directions * self.hidden_dim
            ).transpose(0, 1)
    ...
    def load_model(self, load_dir):
        if self.device.type == "cuda":
            ...
            self.load_state_dict(
                torch.load(
                    open(load_dir, "rb"), map_location=lambda storage, loc: storage
                )
            )
