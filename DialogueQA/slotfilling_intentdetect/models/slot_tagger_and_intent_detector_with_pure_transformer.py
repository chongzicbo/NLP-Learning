...
@author: chengbo
@software: PyCharm
@file: slot_tagger_and_intent_detector_with_pure_transformer.py
@time: 2022/9/7 20:45
"""
...
class Transformers_joint_slot_and_intent(nn.Module):
    def __init__(
        ...
        if self.task_st == "NN":
            ...
            self.hidden2tag = nn.Linear(
                self.embedding_dim + self.append_feature_dim, self.tagset_size + 2
            )
    ...
    def forward(self, sentences, lengths, extFeats=None, masked_output=None):
        ...
        batch_size, transformer_seq_length, hidden_size = (
            transformer_top_hiddens.size(0),
            transformer_top_hiddens.size(1),
            transformer_top_hiddens.size(2),
        )
        chosen_encoder_hiddens = transformer_top_hiddens.view(
            -1, hidden_size
        ...
        # step 2: slot tagger
        concat_input_reshape = concat_input.contiguous().view(
            concat_input.size(0) * concat_input.size(1), concat_input.size(2)
        )
        ...
        tag_scores = tag_scores.view(
            concat_input.size(0), concat_input.size(1), tag_space.size(1)
        )
        ...
        if self.task_sc:
            if self.task_sc == "CLS":
                ...
                hidden_for_intent = torch.cat(
                    (transformer_cls_hidden, embeds.max(1)[0]), dim=1
                )
