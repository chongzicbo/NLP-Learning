...
class LSTMTagger_CRF(nn.Module):
    def __init__(
        ...
        self.hidden2tag = nn.Linear(
            self.num_directions * self.hidden_dim, self.tagset_size + 2
        )
    ...
    def _get_lstm_features(
        self, sentences, lengths, extFeats=None, with_snt_classifier=False
    ):
        ...
        if self.elmo_model and self.pretrained_model:
            ...
            tokens, segments, selects, copies, attention_mask = (
                sentences["transformer"]["tokens"],
                sentences["transformer"]["segments"],
                sentences["transformer"]["selects"],
                sentences["transformer"]["copies"],
                sentences["transformer"]["mask"],
            )
        ...
        packed_lstm_out, packed_h_t_c_t = self.lstm(
            packed_embeds
        ...
        lstm_out_reshape = lstm_out.contiguous().view(
            lstm_out.size(0) * lstm_out.size(1), lstm_out.size(2)
        )
        tag_space = self.hidden2tag(self.dropout_layer(lstm_out_reshape))
        tag_space = tag_space.view(
            lstm_out.size(0), lstm_out.size(1), tag_space.size(1)
        )
