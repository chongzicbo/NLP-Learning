...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query, key, value = [l(x).view(batch_size, -1, self.h, self.d_k).transpose(1, 2) for l, x in
                             zip(self.linear_layers,
                                 (query, key, value))]  # 对q,k,v进行线性变换,分割成多个头[batch_size,d_model]=>[batch_size,h,-1,d_k]

        x, attn = self.attention(query, key, value, mask=mask,
                                 dropout=self.dropout)  # attn:[batch_size,heads,seq_len,seq_len]
